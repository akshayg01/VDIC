...
add_nocache_headers();
include("include/lu_pin_city_state_variables.php");
...
$layout->blocks["top"][] = "details";$page_layouts["lu_pin_city_state_add"] = $layout;
...
if($inlineadd == ADD_INLINE)
	$templatefile = "lu_pin_city_state_inline_add.htm";
else
	$templatefile = "lu_pin_city_state_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: lu_pin_city_state_add.php');
...
if(@$_POST["a"]=="added")
{
	...
	$avalues=array();
	$blobfields=array();
//	processing CountryCode - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_CountryCode = $pageObject->getControl("CountryCode", $id);
		$control_CountryCode->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing CountryCode - end
...
//	processing Locality - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_Locality = $pageObject->getControl("Locality", $id);
		$control_Locality->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing Locality - end
...
//	processing Accuracy - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_Accuracy = $pageObject->getControl("Accuracy", $id);
		$control_Accuracy->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing Accuracy - end
...
//	processing CountryCode - start
			...
			if($inlineAddOption)
			{
				$control_CountryCode->afterSuccessfulSave();
			}
//	processing CountryCode - end
//	processing PINcode - start
			$inlineAddOption = true;
...
//	processing Locality - start
			...
			if($inlineAddOption)
			{
				$control_Locality->afterSuccessfulSave();
			}
//	processing Locality - end
//	processing State - start
			$inlineAddOption = true;
...
//	processing Accuracy - start
			...
			if($inlineAddOption)
			{
				$control_Accuracy->afterSuccessfulSave();
			}
//	processing Accuracy - end
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'lu_pin_city_state_edit.php?'.$keylink.'\'>'."Edit".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'lu_pin_city_state_view.php?'.$keylink.'\'>'."View".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: lu_pin_city_state_".$pageObject->getPageType().".php");
...
if(postvalue("table")=="devotee_address" && postvalue("field")=="City") 
	$defvalues["State"] = postvalue("category");
if(postvalue("table")=="company" && postvalue("field")=="City") 
	$defvalues["State"] = postvalue("category");
if($readavalues)
{
	$defvalues["CountryCode"]=@$avalues["CountryCode"];
	$defvalues["PINcode"]=@$avalues["PINcode"];
	$defvalues["Locality"]=@$avalues["Locality"];
	...
	$defvalues["Accuracy"]=@$avalues["Accuracy"];
}
...
if($inlineadd!=ADD_INLINE)
{
	...
	if(!$pageObject->isAppearOnTabs("CountryCode"))
		$xt->assign("CountryCode_fieldblock",true);
	else
		$xt->assign("CountryCode_tabfieldblock",true);
	$xt->assign("CountryCode_label",true);
	if(isEnableSection508())
		$xt->assign_section("CountryCode_label","<label for=\"".GetInputElementId("CountryCode", $id, PAGE_ADD)."\">","</label>");
	...
	if(!$pageObject->isAppearOnTabs("Locality"))
		$xt->assign("Locality_fieldblock",true);
	else
		$xt->assign("Locality_tabfieldblock",true);
	$xt->assign("Locality_label",true);
	if(isEnableSection508())
		$xt->assign_section("Locality_label","<label for=\"".GetInputElementId("Locality", $id, PAGE_ADD)."\">","</label>");
	...
	if(!$pageObject->isAppearOnTabs("Accuracy"))
		$xt->assign("Accuracy_fieldblock",true);
	else
		$xt->assign("Accuracy_tabfieldblock",true);
	$xt->assign("Accuracy_label",true);
	if(isEnableSection508())
		$xt->assign_section("Accuracy_label","<label for=\"".GetInputElementId("Accuracy", $id, PAGE_ADD)."\">","</label>");
...
//	CountryCode
	...
	if($display)
	{	
		$value = $pageObject->showDBValue("CountryCode", $data, $keylink);
		$showValues["CountryCode"] = $value;
		$showFields[] = "CountryCode";
	}	
//	PINcode
	$display = false;
	if($inlineadd==ADD_MASTER)
...
//	Locality
	...
	if($display)
	{	
		$value = $pageObject->showDBValue("Locality", $data, $keylink);
		$showValues["Locality"] = $value;
		$showFields[] = "Locality";
	}	
//	State
	$display = false;
...
//	Accuracy
	...
	if($display)
	{	
		$value = $pageObject->showDBValue("Accuracy", $data, $keylink);
		$showValues["Accuracy"] = $value;
		$showFields[] = "Accuracy";
	}	
		$showRawValues["CountryCode"] = substr($data["CountryCode"],0,100);
		$showRawValues["PINcode"] = substr($data["PINcode"],0,100);
		$showRawValues["Locality"] = substr($data["Locality"],0,100);
		...
		$showRawValues["Accuracy"] = substr($data["Accuracy"],0,100);
...
if($pageObject->isShowDetailTables && ($inlineadd==ADD_SIMPLE || $inlineadd==ADD_POPUP) && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "lu_pin_city_state";
		$options['firstTime'] = 1;
...
function GetAddedDataLookupQuery($pageObject, $keys, $forLookup)
{
	...
	if(postvalue("table")=="company" && postvalue("field")=="City") 
	{
		$lookupFieldName = "City";
		$lookupTable = "lu_pin_city_state";
		$linkFieldName = "City";
		$dispfield = "City";
		...
		if($nLookupType == LT_QUERY)
		{
			if($lookupPSet->getCustomDisplay($lookupFieldName))
				$pageObject->pSet->getSQLQuery()->AddCustomExpression($dispfield, $pageObject->pSet, "company", "City");
			...
			$LookupSQL = "select ";
			$LookupSQL .= GetFullFieldName("City", $strTableName, true);
