...
(function(){
	...
	var isIE9 = (!isOpera && userAgent.indexOf("msie 9") > -1); 
	...
	Runner.apply(Runner, {
		 ...
		 */
		setLeftTop: function(link, preview, viewContainer, doNotOverlap, pageObj){

			if (pageObj && !pageObj.masterDetails.show){
				return;
			} 
			...
			var scrollBarWidth = 16,
				previewObj = $(preview),
				linkObj = $(link),
				linkOffset = linkObj.offset(),
				linkAbsPos = { left: linkOffset.left, top: linkOffset.top, width: linkObj.width(), height: linkObj.height()}, //Runner.getAbsolutePosition(link, 1),
			// extra offset
				objectOffsetFromBorders = 5,
				newTop = 0, newLeft = 0,
			// element coordinates
			// viewport dimensions  
			...
			// viewport X dimension
				viewBottomX = clientWidth - scrollBarWidth - objectOffsetFromBorders + $(window).scrollLeft(),
			// element X dimension
				elementBottomX = linkAbsPos.left + linkAbsPos.width + previewObj.width(),
			// viewport Y dimension
				viewBottomY = clientHeight - scrollBarWidth - objectOffsetFromBorders + $(window).scrollTop();
			
			// if part of element situated outside view port
			if (elementBottomX > viewBottomX){
				// make offset for element
				newLeft = linkAbsPos.left + linkAbsPos.width - (elementBottomX - viewBottomX) - objectOffsetFromBorders - scrollBarWidth;
				if (newLeft < 0){
					newLeft = 0;
				}
				newTop = linkAbsPos.top + linkAbsPos.height + 5;
			}else{
				newLeft = linkAbsPos.left + linkAbsPos.width + 5;
				newTop = linkAbsPos.top;
			}
			var elementBottomY = newTop + previewObj.height();
			...
			if (elementBottomY > viewBottomY){
				// make offset for element
				if(doNotOverlap){
					newTop = linkAbsPos.top - previewObj.height() - 5;
					if (newTop < 0){
						newTop = linkAbsPos.top + linkAbsPos.height + 5;
					}
				}else{
					newTop = linkAbsPos.top + linkAbsPos.height - (elementBottomY - viewBottomY) - objectOffsetFromBorders - scrollBarWidth;
					if (newTop < 0){
						newTop = 0;
					}
				}
			...
			previewObj.css({"left": newLeft + "px", "top": newTop + "px"});
		},
		 ...
		 * @param {DOM element} container wich contain link (body or fly window)
		 */
		setLeftTopRTL: function(link, preview, viewContainer, doNotOverlap, pageObj){
			...
			var scrollBarWidth = 16,
				...
				linkObj = $(link),
				linkOffset = linkObj.offset(),
				linkAbsPos = { left: linkOffset.left, top: linkOffset.top, width: linkObj.width(), height: linkObj.height()},
				divInnerWidth = Runner.isIE ? previewObj.width() : previewObj.innerWidth(),
			// extra offset
				objectOffsetFromBorders = 5,
			// viewport dimensions  
				clientHeight = $(window).height(),
			...
			// searching new left coordinates for popub div
			// viewport X dimension
				viewBottomX = objectOffsetFromBorders,
			// element X dimension
				elementBottomX = linkAbsPos.left - divInnerWidth - 5,
			// viewport Y dimension
				viewBottomY = clientHeight - scrollBarWidth - objectOffsetFromBorders + $(window).scrollTop();
			
			if(Runner.isIE){
				linkAbsPos.left = -20 + this.getSimpleOffsetForIE(link);
				elementBottomX = linkAbsPos.left - divInnerWidth - 5;
			}
			
			// if part of element situated outside view port
			if (viewBottomX > linkAbsPos.left){
				// make offset for element
				newX = viewBottomX + divInnerWidth;
				newY = linkAbsPos.top + linkAbsPos.height + 5;
			}else{
				newX = linkAbsPos.left - 5 - divInnerWidth;
				newY = linkAbsPos.top;
			}
			
			var elementBottomY = newY + previewObj.height();
			...
			if (elementBottomY > viewBottomY){
				// make offset for element
				if(doNotOverlap){
					newY = linkAbsPos.top - previewObj.height() - 5;
					if (newY < 0){
						newY = linkAbsPos.top + linkAbsPos.height + 5;
					}
				}else{
					newY = linkAbsPos.top + linkAbsPos.height - (elementBottomY - viewBottomY) - objectOffsetFromBorders - scrollBarWidth;
					if (newY < 0){
						newY = 0;
					}
				}
			...
			previewObj.css({"left": newX + "px", "top": newY + "px"});
		},
		
		getSimpleOffsetForIE: function(element){
			var offsetParent = element.offsetParent, result = element.offsetLeft;
			while(typeof offsetParent != "undefined" && offsetParent != null){
				result += offsetParent.offsetLeft;
		...
		resizeLoadingDivAfterPageScroll: function(){
			var loadFon = $(".runner-loading .fon");
			if(loadFon.length > 0){
		 ...
		 */
		runLoading: function(root){
			if(!root){
				root = document.body;
			}
			if(typeof root.loadingRunned == undefined)
		...
		getLoadingBlock: function(){
			return	'<div class="main runner-panel">'+
		...
		// Why do it through the window object????
		...
		isAcceptableKeyCode: function(oEvent){
			...
			return !illegalKeys.isInArray(oEvent.keyCode, true);
		},
		
		/**
		 * Return true if passed page or any of its parents are flying
		 */
		 ...
		 * (Moved from RunnerPage for possibility to invoke from everywhere)
		 */
...
Array.prototype.serialize = function(mixed_value){
