...
include("include/lu_family_relationship_variables.php");
...
$layout->blocks["top"][] = "details";$page_layouts["lu_family_relationship_edit"] = $layout;
...
$templatefile = ($inlineedit == EDIT_INLINE) ? "lu_family_relationship_inline_edit.htm" : "lu_family_relationship_edit.htm";
...
$skeys = "";
$savedKeys = array();
$keys["RelationshipId"] = urldecode(postvalue("editid1"));
$savedKeys["RelationshipId"] = urldecode(postvalue("editid1"));
...
//	processing RelationshipId - begin
	$condition = 1;

	if($condition)
	{
		$control_RelationshipId = $pageObject->getControl("RelationshipId", $id);
		$control_RelationshipId->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		//	update key value
		if($control_RelationshipId->getWebValue()!==false)
			$keys["RelationshipId"] = $control_RelationshipId->getWebValue();
	}
//	processing RelationshipId - end
//	processing RelationshipName - begin
	$condition = 1;

	if($condition)
	{
		$control_RelationshipName = $pageObject->getControl("RelationshipName", $id);
		$control_RelationshipName->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing RelationshipName - end
	...
	//	do event
		...
		{		
			...
			//	processing RelationshipId - begin
				...
				if($condition)
				{
					$control_RelationshipId->afterSuccessfulSave();
				}
	//	processing RelationshipId - end
			//	processing RelationshipName - begin
							$condition = 1;
				...
				if($condition)
				{
					$control_RelationshipName->afterSuccessfulSave();
				}
	//	processing RelationshipName - end
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// key get query
	$keyGetQ = "";
		$keyGetQ.="editid1=".rawurldecode($keys["RelationshipId"])."&";
	// cut last &
	$keyGetQ = substr($keyGetQ, 0, strlen($keyGetQ)-1);	
	// redirect
	header("Location: lu_family_relationship_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: lu_family_relationship_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
if($readevalues)
{
	$data["RelationshipId"] = $evalues["RelationshipId"];
	$data["RelationshipName"] = $evalues["RelationshipName"];
}
...
if($inlineedit != EDIT_INLINE)
{
	...
	if(!$pageObject->isAppearOnTabs("RelationshipId"))
		$xt->assign("RelationshipId_fieldblock",true);
	else
		$xt->assign("RelationshipId_tabfieldblock",true);
	$xt->assign("RelationshipId_label",true);
	if(isEnableSection508())
		$xt->assign_section("RelationshipId_label","<label for=\"".GetInputElementId("RelationshipId", $id, PAGE_EDIT)."\">","</label>");
		
	if(!$pageObject->isAppearOnTabs("RelationshipName"))
		$xt->assign("RelationshipName_fieldblock",true);
	else
		$xt->assign("RelationshipName_tabfieldblock",true);
	$xt->assign("RelationshipName_label",true);
	if(isEnableSection508())
		$xt->assign_section("RelationshipName_label","<label for=\"".GetInputElementId("RelationshipName", $id, PAGE_EDIT)."\">","</label>");
		

	$xt->assign("show_key1", htmlspecialchars($pageObject->showDBValue("RelationshipId", $data)));
...
//	detail tables
	...
	$keylink = "";
	$keylink.= "&key1=".htmlspecialchars(rawurlencode(@$data["RelationshipId"]));


//	RelationshipId - 
	$value = $pageObject->showDBValue("RelationshipId", $data, $keylink);
	$showValues["RelationshipId"] = $value;
	$showFields[] = "RelationshipId";
		$showRawValues["RelationshipId"] = substr($data["RelationshipId"],0,100);

//	RelationshipName - 
	$value = $pageObject->showDBValue("RelationshipName", $data, $keylink);
	$showValues["RelationshipName"] = $value;
	$showFields[] = "RelationshipName";
		$showRawValues["RelationshipName"] = substr($data["RelationshipName"],0,100);
/////////////////////////////////////////////////////////////
//	start inline output
/////////////////////////////////////////////////////////////
...
if($pageObject->isShowDetailTables && $inlineedit!=EDIT_INLINE && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "lu_family_relationship";
		$options['firstTime'] = 1;
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='lu_family_relationship_view.php?".$viewlink."'\"");
