...
$layout->blocks["top"][] = "details";$page_layouts["devotee_org_add"] = $layout;
...
if($inlineadd == ADD_INLINE)
	$templatefile = "devotee_org_inline_add.htm";
else
	$templatefile = "devotee_org_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: devotee_org_add.php');
...
if(@$_POST["a"]=="added")
{
	...
	$avalues=array();
	$blobfields=array();
//	processing OrgId - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_OrgId = $pageObject->getControl("OrgId", $id);
		$control_OrgId->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
...
//	processing Role - start
	...
	if($inlineAddOption)
	{
		...
		$control_Role->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing Role - end
...
//	insert masterkey value if exists and if not specified
	...
	if(@$_SESSION[$sessionPrefix."_mastertable"]=="organization")
	{
		if(postvalue("masterkey1"))
			$_SESSION[$sessionPrefix."_masterkey1"] = postvalue("masterkey1");
		
		if($avalues["OrgId"]==""){
			$avalues["OrgId"] = prepare_for_db("OrgId",$_SESSION[$sessionPrefix."_masterkey1"]);
		}
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'devotee_org_edit.php?'.$keylink.'\'>'."Edit".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'devotee_org_view.php?'.$keylink.'\'>'."View".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: devotee_org_".$pageObject->getPageType().".php");
...
if(array_key_exists("copyid1",$_REQUEST) || array_key_exists("editid1",$_REQUEST))
{
	...
	{
		$copykeys["DevoteeOrgId"]=postvalue("copyid1");
	}
	else
	{
		$copykeys["DevoteeOrgId"]=postvalue("editid1");
	}
...
if(@$_SESSION[$sessionPrefix."_mastertable"]=="organization")
{
	...
	if(postvalue("mainMPageType")<>"add")
		$defvalues["OrgId"] = @$_SESSION[$sessionPrefix."_masterkey1"];	
	
}

if($readavalues)
{
	$defvalues["OrgId"]=@$avalues["OrgId"];
	$defvalues["FromDate"]=@$avalues["FromDate"];
	$defvalues["ToDate"]=@$avalues["ToDate"];
	$defvalues["Role"]=@$avalues["Role"];
}
...
if($inlineadd!=ADD_INLINE)
{
	...
	if(isEnableSection508())
		$xt->assign_section("OrgId_label","<label for=\"".GetInputElementId("OrgId", $id, PAGE_ADD)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("Role_label","<label for=\"".GetInputElementId("Role", $id, PAGE_ADD)."\">","</label>");
