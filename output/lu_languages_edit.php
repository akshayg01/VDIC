...
$layout->blocks["top"][] = "details";$page_layouts["lu_languages_edit"] = $layout;
...
$templatefile = ($inlineedit == EDIT_INLINE) ? "lu_languages_inline_edit.htm" : "lu_languages_edit.htm";
...
$skeys = "";
$savedKeys = array();
$keys["LanguageId"] = urldecode(postvalue("editid1"));
$savedKeys["LanguageId"] = urldecode(postvalue("editid1"));
...
//	processing LanguageId - begin
	...
	if($condition)
	{
		...
		//	update key value
		if($control_LanguageId->getWebValue()!==false)
			$keys["LanguageId"] = $control_LanguageId->getWebValue();
	}
//	processing LanguageId - end
//	processing language - begin
	$condition = 1;

	if($condition)
	{
		$control_language = $pageObject->getControl("language", $id);
		$control_language->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// key get query
	$keyGetQ = "";
		$keyGetQ.="editid1=".rawurldecode($keys["LanguageId"])."&";
	// cut last &
	$keyGetQ = substr($keyGetQ, 0, strlen($keyGetQ)-1);	
	// redirect
	header("Location: lu_languages_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: lu_languages_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
if($inlineedit != EDIT_INLINE)
{
	...
	if(isEnableSection508())
		$xt->assign_section("LanguageId_label","<label for=\"".GetInputElementId("LanguageId", $id, PAGE_EDIT)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("language_label","<label for=\"".GetInputElementId("language", $id, PAGE_EDIT)."\">","</label>");
		

	$xt->assign("show_key1", htmlspecialchars($pageObject->showDBValue("LanguageId", $data)));
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='lu_languages_view.php?".$viewlink."'\"");
