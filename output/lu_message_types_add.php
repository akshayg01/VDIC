...
$layout->blocks["top"][] = "details";$page_layouts["lu_message_types_add"] = $layout;
...
if($inlineadd == ADD_INLINE)
	$templatefile = "lu_message_types_inline_add.htm";
else
	$templatefile = "lu_message_types_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: lu_message_types_add.php');
...
if(@$_POST["a"]=="added")
{
	...
	$avalues=array();
	$blobfields=array();
//	processing MessageTypeId - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_MessageTypeId = $pageObject->getControl("MessageTypeId", $id);
		$control_MessageTypeId->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing MessageTypeId - end
//	processing MessageType - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_MessageType = $pageObject->getControl("MessageType", $id);
		$control_MessageType->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'lu_message_types_edit.php?'.$keylink.'\'>'."Edit".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'lu_message_types_view.php?'.$keylink.'\'>'."View".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: lu_message_types_".$pageObject->getPageType().".php");
...
if(array_key_exists("copyid1",$_REQUEST) || array_key_exists("editid1",$_REQUEST))
{
	...
	{
		$copykeys["MessageTypeId"]=postvalue("copyid1");
	}
	else
	{
		$copykeys["MessageTypeId"]=postvalue("editid1");
	}
...
if($readavalues)
{
	$defvalues["MessageTypeId"]=@$avalues["MessageTypeId"];
	$defvalues["MessageType"]=@$avalues["MessageType"];
}
...
if($inlineadd!=ADD_INLINE)
{
	...
	if(isEnableSection508())
		$xt->assign_section("MessageTypeId_label","<label for=\"".GetInputElementId("MessageTypeId", $id, PAGE_ADD)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("MessageType_label","<label for=\"".GetInputElementId("MessageType", $id, PAGE_ADD)."\">","</label>");
