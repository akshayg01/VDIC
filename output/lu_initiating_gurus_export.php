...
$layout->blocks["top"][] = "export";$page_layouts["lu_initiating_gurus_export"] = $layout;
...
//	process selection
	...
	if (@$_REQUEST["mdelete"])
	{
		...
		{
			$keys=array();
			$keys["GurumaharajId"] = refine($_REQUEST["mdelete1"][mdeleteIndex($ind)]);
			$selected_recs[] = $keys;
		}
	}
	elseif(@$_REQUEST["selection"])
	{
		foreach(@$_REQUEST["selection"] as $keyblock)
		{
			...
			if(count($arr)<1)
				continue;
			$keys = array();
			$keys["GurumaharajId"] = urldecode($arr[0]);
			$selected_recs[] = $keys;
		}
	}
...
$xt->display("lu_initiating_gurus_export.htm");

function ExportToExcel_old($cipherer)
{
	global $cCharset;
	header("Content-Type: application/vnd.ms-excel");
	header("Content-Disposition: attachment;Filename=lu_initiating_gurus.xls");
...
function ExportToWord($cipherer)
{
	...
	header("Content-Disposition: attachment;Filename=lu_initiating_gurus.doc");
...
function ExportToXML($cipherer)
{
	...
	header("Content-Disposition: attachment;Filename=lu_initiating_gurus.xml");
	...
	{
		$values = array();
			$values["GurumaharajId"] = $pageObject->showDBValue("GurumaharajId", $row);
...
function ExportToCSV($cipherer)
{
	...
	header("Content-Disposition: attachment;Filename=lu_initiating_gurus.csv");
...
// write data rows
	...
	{
		$values = array();
			$values["GurumaharajId"] = $pageObject->getViewControl("GurumaharajId")->showDBValue($row, "");
		...
		if ($eventRes)
		{
			...
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["GurumaharajId"]).'"';
			if($outstr!="")
				$outstr.=",";
...
// write header
	...
	{
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Gurumaharaj Id").'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Name").'</td>';	
	}
	else
	{
		echo "<td>"."Gurumaharaj Id"."</td>";
		echo "<td>"."Name"."</td>";
	}
	echo "</tr>";
