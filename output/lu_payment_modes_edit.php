...
include("include/lu_payment_modes_variables.php");
...
$layout->blocks["top"][] = "details";$page_layouts["lu_payment_modes_edit"] = $layout;
...
$templatefile = ($inlineedit == EDIT_INLINE) ? "lu_payment_modes_inline_edit.htm" : "lu_payment_modes_edit.htm";
...
$skeys = "";
$savedKeys = array();
$keys["PaymentModeId"] = urldecode(postvalue("editid1"));
$savedKeys["PaymentModeId"] = urldecode(postvalue("editid1"));
...
//	processing PaymentModeId - begin
	$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode

	if($condition)
	{
		$control_PaymentModeId = $pageObject->getControl("PaymentModeId", $id);
		$control_PaymentModeId->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		//	update key value
		if($control_PaymentModeId->getWebValue()!==false)
			$keys["PaymentModeId"] = $control_PaymentModeId->getWebValue();
	}
//	processing PaymentModeId - end
	...
	//	do event
		...
		{		
			...
			//	processing PaymentModeId - begin
				...
				if($condition)
				{
					$control_PaymentModeId->afterSuccessfulSave();
				}
	//	processing PaymentModeId - end
			//	processing Mode - begin
							$condition = 1;
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// key get query
	$keyGetQ = "";
		$keyGetQ.="editid1=".rawurldecode($keys["PaymentModeId"])."&";
	// cut last &
	$keyGetQ = substr($keyGetQ, 0, strlen($keyGetQ)-1);	
	// redirect
	header("Location: lu_payment_modes_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: lu_payment_modes_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
if($readevalues)
{
	$data["PaymentModeId"] = $evalues["PaymentModeId"];
...
if($inlineedit != EDIT_INLINE)
{
	...
	$xt->assign("show_key1", htmlspecialchars($pageObject->showDBValue("PaymentModeId", $data)));
...
//	detail tables
	...
	$keylink = "";
	$keylink.= "&key1=".htmlspecialchars(rawurlencode(@$data["PaymentModeId"]));


//	PaymentModeId - 
	$value = $pageObject->showDBValue("PaymentModeId", $data, $keylink);
	$showValues["PaymentModeId"] = $value;
	$showFields[] = "PaymentModeId";
		$showRawValues["PaymentModeId"] = substr($data["PaymentModeId"],0,100);
...
if($pageObject->isShowDetailTables && $inlineedit!=EDIT_INLINE && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "lu_payment_modes";
		$options['firstTime'] = 1;
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='lu_payment_modes_view.php?".$viewlink."'\"");
