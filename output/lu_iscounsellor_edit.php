...
$layout->blocks["top"][] = "details";$page_layouts["lu_iscounsellor_edit"] = $layout;
...
$templatefile = ($inlineedit == EDIT_INLINE) ? "lu_iscounsellor_inline_edit.htm" : "lu_iscounsellor_edit.htm";
...
$skeys = "";
$savedKeys = array();
$keys["IsCounsellorId"] = urldecode(postvalue("editid1"));
$savedKeys["IsCounsellorId"] = urldecode(postvalue("editid1"));
...
//	processing IsCounsellor - begin
	$condition = 1;

	if($condition)
	{
		$control_IsCounsellor = $pageObject->getControl("IsCounsellor", $id);
		$control_IsCounsellor->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// key get query
	$keyGetQ = "";
		$keyGetQ.="editid1=".rawurldecode($keys["IsCounsellorId"])."&";
	// cut last &
	$keyGetQ = substr($keyGetQ, 0, strlen($keyGetQ)-1);	
	// redirect
	header("Location: lu_iscounsellor_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: lu_iscounsellor_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
if($readevalues)
{
	$data["IsCounsellor"] = $evalues["IsCounsellor"];
}
...
if($inlineedit != EDIT_INLINE)
{
	...
	if(isEnableSection508())
		$xt->assign_section("IsCounsellor_label","<label for=\"".GetInputElementId("IsCounsellor", $id, PAGE_EDIT)."\">","</label>");
		

	$xt->assign("show_key1", htmlspecialchars($pageObject->showDBValue("IsCounsellorId", $data)));
...
//	detail tables
	...
	$keylink = "";
	$keylink.= "&key1=".htmlspecialchars(rawurlencode(@$data["IsCounsellorId"]));


//	IsCounsellorId - 
	$value = $pageObject->showDBValue("IsCounsellorId", $data, $keylink);
	$showValues["IsCounsellorId"] = $value;
	$showFields[] = "IsCounsellorId";
		$showRawValues["IsCounsellorId"] = substr($data["IsCounsellorId"],0,100);
...
if($pageObject->isShowDetailTables && $inlineedit!=EDIT_INLINE && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "lu_iscounsellor";
		$options['firstTime'] = 1;
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='lu_iscounsellor_view.php?".$viewlink."'\"");
