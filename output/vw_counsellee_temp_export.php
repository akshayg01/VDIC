...
$layout->blocks["top"][] = "export";$page_layouts["vw_counsellee_temp_export"] = $layout;
...
$xt->display("vw_counsellee_temp_export.htm");

function ExportToExcel_old($cipherer)
{
	global $cCharset;
	header("Content-Type: application/vnd.ms-excel");
	header("Content-Disposition: attachment;Filename=vw_counsellee_temp.xls");
...
function ExportToWord($cipherer)
{
	...
	header("Content-Disposition: attachment;Filename=vw_counsellee_temp.doc");
...
function ExportToXML($cipherer)
{
	...
	header("Content-Disposition: attachment;Filename=vw_counsellee_temp.xml");
	...
	while((!$nPageSize || $i<$nPageSize) && $row)
	{
		$values = array();
			...
			$values["DateOfHarinamInitiation"] = $pageObject->showDBValue("DateOfHarinamInitiation", $row);
...
function ExportToCSV($cipherer)
{
	...
	header("Content-Disposition: attachment;Filename=vw_counsellee_temp.csv");
...
// write header
	...
	if($outstr!="")
		$outstr.=",";
	$outstr.= "\"DateOfHarinamInitiation\"";
	echo $outstr;
	echo "\r\n";
...
// write data rows
	...
	while((!$nPageSize || $iNumberOfRows < $nPageSize) && $row)
	{
		...
		$values["Photo"] = "LONG BINARY DATA - CANNOT BE DISPLAYED";
			...
			$values["DateOfHarinamInitiation"] = $pageObject->getViewControl("DateOfHarinamInitiation")->showDBValue($row, "");
		...
		if ($eventRes)
		{
			...
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["DateOfHarinamInitiation"]).'"';
			echo $outstr;
		}
		...
		if ($eventRes)
		{
			...
			echo '</td>';
							echo '<td>';
									...
									if($_REQUEST["type"]=="excel")
						echo PrepareForExcel($values["DateOfHarinamInitiation"]);
					else
						echo $values["DateOfHarinamInitiation"];
			echo '</td>';
			echo "</tr>";
		}
