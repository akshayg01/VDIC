...
$layout->blocks["top"][] = "details";$page_layouts["lu_departments_add"] = $layout;
...
if($inlineadd == ADD_INLINE)
	$templatefile = "lu_departments_inline_add.htm";
else
	$templatefile = "lu_departments_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: lu_departments_add.php');
...
if(@$_POST["a"]=="added")
{
	...
	$avalues=array();
	$blobfields=array();
//	processing DepartmentId - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_DepartmentId = $pageObject->getControl("DepartmentId", $id);
		$control_DepartmentId->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing DepartmentId - end
//	processing DepartmentName - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_DepartmentName = $pageObject->getControl("DepartmentName", $id);
		$control_DepartmentName->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing DepartmentName - end
//	processing DepartmentInCharge - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_DepartmentInCharge = $pageObject->getControl("DepartmentInCharge", $id);
		$control_DepartmentInCharge->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'lu_departments_edit.php?'.$keylink.'\'>'."Edit".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'lu_departments_view.php?'.$keylink.'\'>'."View".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: lu_departments_".$pageObject->getPageType().".php");
...
if(array_key_exists("copyid1",$_REQUEST) || array_key_exists("editid1",$_REQUEST))
{
	...
	{
		$copykeys["DepartmentId"]=postvalue("copyid1");
	}
	else
	{
		$copykeys["DepartmentId"]=postvalue("editid1");
	}
...
if($readavalues)
{
	$defvalues["DepartmentId"]=@$avalues["DepartmentId"];
	$defvalues["DepartmentName"]=@$avalues["DepartmentName"];
	$defvalues["DepartmentInCharge"]=@$avalues["DepartmentInCharge"];
}
...
if($inlineadd!=ADD_INLINE)
{
	...
	if(isEnableSection508())
		$xt->assign_section("DepartmentId_label","<label for=\"".GetInputElementId("DepartmentId", $id, PAGE_ADD)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("DepartmentName_label","<label for=\"".GetInputElementId("DepartmentName", $id, PAGE_ADD)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("DepartmentInCharge_label","<label for=\"".GetInputElementId("DepartmentInCharge", $id, PAGE_ADD)."\">","</label>");
