...
$layout->blocks["top"][] = "details";$page_layouts["lu_departments_edit"] = $layout;
...
$templatefile = ($inlineedit == EDIT_INLINE) ? "lu_departments_inline_edit.htm" : "lu_departments_edit.htm";
...
$skeys = "";
$savedKeys = array();
$keys["DepartmentId"] = urldecode(postvalue("editid1"));
$savedKeys["DepartmentId"] = urldecode(postvalue("editid1"));
...
//	processing DepartmentId - begin
	$condition = 1;

	if($condition)
	{
		$control_DepartmentId = $pageObject->getControl("DepartmentId", $id);
		$control_DepartmentId->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		//	update key value
		if($control_DepartmentId->getWebValue()!==false)
			$keys["DepartmentId"] = $control_DepartmentId->getWebValue();
	}
//	processing DepartmentId - end
//	processing DepartmentName - begin
	$condition = 1;

	if($condition)
	{
		$control_DepartmentName = $pageObject->getControl("DepartmentName", $id);
		$control_DepartmentName->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing DepartmentName - end
//	processing DepartmentInCharge - begin
	$condition = 1;

	if($condition)
	{
		$control_DepartmentInCharge = $pageObject->getControl("DepartmentInCharge", $id);
		$control_DepartmentInCharge->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing DepartmentInCharge - end
	...
	//	do event
		...
		{		
			...
			//	processing DepartmentId - begin
				...
				if($condition)
				{
					$control_DepartmentId->afterSuccessfulSave();
				}
	//	processing DepartmentId - end
			//	processing DepartmentName - begin
							$condition = 1;
				...
				if($condition)
				{
					$control_DepartmentName->afterSuccessfulSave();
				}
	//	processing DepartmentName - end
			//	processing DepartmentInCharge - begin
							$condition = 1;
				...
				if($condition)
				{
					$control_DepartmentInCharge->afterSuccessfulSave();
				}
	//	processing DepartmentInCharge - end
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// key get query
	$keyGetQ = "";
		$keyGetQ.="editid1=".rawurldecode($keys["DepartmentId"])."&";
	// cut last &
	$keyGetQ = substr($keyGetQ, 0, strlen($keyGetQ)-1);	
	// redirect
	header("Location: lu_departments_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: lu_departments_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
if($readevalues)
{
	$data["DepartmentId"] = $evalues["DepartmentId"];
	$data["DepartmentName"] = $evalues["DepartmentName"];
	$data["DepartmentInCharge"] = $evalues["DepartmentInCharge"];
}
...
if($inlineedit != EDIT_INLINE)
{
	...
	if(!$pageObject->isAppearOnTabs("DepartmentId"))
		$xt->assign("DepartmentId_fieldblock",true);
	else
		$xt->assign("DepartmentId_tabfieldblock",true);
	$xt->assign("DepartmentId_label",true);
	if(isEnableSection508())
		$xt->assign_section("DepartmentId_label","<label for=\"".GetInputElementId("DepartmentId", $id, PAGE_EDIT)."\">","</label>");
		
	if(!$pageObject->isAppearOnTabs("DepartmentName"))
		$xt->assign("DepartmentName_fieldblock",true);
	else
		$xt->assign("DepartmentName_tabfieldblock",true);
	$xt->assign("DepartmentName_label",true);
	if(isEnableSection508())
		$xt->assign_section("DepartmentName_label","<label for=\"".GetInputElementId("DepartmentName", $id, PAGE_EDIT)."\">","</label>");
		
	if(!$pageObject->isAppearOnTabs("DepartmentInCharge"))
		$xt->assign("DepartmentInCharge_fieldblock",true);
	else
		$xt->assign("DepartmentInCharge_tabfieldblock",true);
	$xt->assign("DepartmentInCharge_label",true);
	if(isEnableSection508())
		$xt->assign_section("DepartmentInCharge_label","<label for=\"".GetInputElementId("DepartmentInCharge", $id, PAGE_EDIT)."\">","</label>");
		

	$xt->assign("show_key1", htmlspecialchars($pageObject->showDBValue("DepartmentId", $data)));
...
//	detail tables
	...
	$keylink = "";
	$keylink.= "&key1=".htmlspecialchars(rawurlencode(@$data["DepartmentId"]));


//	DepartmentId - 
	$value = $pageObject->showDBValue("DepartmentId", $data, $keylink);
	$showValues["DepartmentId"] = $value;
	$showFields[] = "DepartmentId";
		$showRawValues["DepartmentId"] = substr($data["DepartmentId"],0,100);

//	DepartmentName - 
	$value = $pageObject->showDBValue("DepartmentName", $data, $keylink);
	$showValues["DepartmentName"] = $value;
	$showFields[] = "DepartmentName";
		$showRawValues["DepartmentName"] = substr($data["DepartmentName"],0,100);

//	DepartmentInCharge - 
	$value = $pageObject->showDBValue("DepartmentInCharge", $data, $keylink);
	$showValues["DepartmentInCharge"] = $value;
	$showFields[] = "DepartmentInCharge";
		$showRawValues["DepartmentInCharge"] = substr($data["DepartmentInCharge"],0,100);
/////////////////////////////////////////////////////////////
//	start inline output
/////////////////////////////////////////////////////////////
...
if($pageObject->isShowDetailTables && $inlineedit!=EDIT_INLINE && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "lu_departments";
		$options['firstTime'] = 1;
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='lu_departments_view.php?".$viewlink."'\"");
