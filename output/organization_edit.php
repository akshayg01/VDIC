...
$layout->blocks["top"][] = "details";$page_layouts["organization_edit"] = $layout;
...
$templatefile = ($inlineedit == EDIT_INLINE) ? "organization_inline_edit.htm" : "organization_edit.htm";
...
$skeys = "";
$savedKeys = array();
$keys["OrgId"] = urldecode(postvalue("editid1"));
$savedKeys["OrgId"] = urldecode(postvalue("editid1"));
...
//	processing OrgId - begin
	...
	if($condition)
	{
		...
		//	update key value
		if($control_OrgId->getWebValue()!==false)
			$keys["OrgId"] = $control_OrgId->getWebValue();
	}
//	processing OrgId - end
//	processing OrgName - begin
	$condition = 1;

	if($condition)
	{
		$control_OrgName = $pageObject->getControl("OrgName", $id);
		$control_OrgName->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing OrgName - end
//	processing OrgOwnerId - begin
	$condition = 1;

	if($condition)
	{
		$control_OrgOwnerId = $pageObject->getControl("OrgOwnerId", $id);
		$control_OrgOwnerId->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// key get query
	$keyGetQ = "";
		$keyGetQ.="editid1=".rawurldecode($keys["OrgId"])."&";
	// cut last &
	$keyGetQ = substr($keyGetQ, 0, strlen($keyGetQ)-1);	
	// redirect
	header("Location: organization_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: organization_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
if($readevalues)
{
	...
	$data["OrgName"] = $evalues["OrgName"];
	$data["OrgOwnerId"] = $evalues["OrgOwnerId"];
...
if($inlineedit != EDIT_INLINE)
{
	...
	if(isEnableSection508())
		$xt->assign_section("OrgName_label","<label for=\"".GetInputElementId("OrgName", $id, PAGE_EDIT)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("OrgOwnerId_label","<label for=\"".GetInputElementId("OrgOwnerId", $id, PAGE_EDIT)."\">","</label>");
	...
	$xt->assign("show_key1", htmlspecialchars($pageObject->showDBValue("OrgId", $data)));
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='organization_view.php?".$viewlink."'\"");
