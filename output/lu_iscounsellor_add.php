...
$layout->blocks["top"][] = "details";$page_layouts["lu_iscounsellor_add"] = $layout;
...
if($inlineadd == ADD_INLINE)
	$templatefile = "lu_iscounsellor_inline_add.htm";
else
	$templatefile = "lu_iscounsellor_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: lu_iscounsellor_add.php');
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'lu_iscounsellor_edit.php?'.$keylink.'\'>'."Edit".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'lu_iscounsellor_view.php?'.$keylink.'\'>'."View".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: lu_iscounsellor_".$pageObject->getPageType().".php");
...
if(array_key_exists("copyid1",$_REQUEST) || array_key_exists("editid1",$_REQUEST))
{
	...
	{
		$copykeys["IsCounsellorId"]=postvalue("copyid1");
	}
	else
	{
		$copykeys["IsCounsellorId"]=postvalue("editid1");
	}
