...
include("include/lu_initiating_gurus_variables.php");
...
$layout->blocks["top"][] = "details";$page_layouts["lu_initiating_gurus_edit"] = $layout;
...
$templatefile = ($inlineedit == EDIT_INLINE) ? "lu_initiating_gurus_inline_edit.htm" : "lu_initiating_gurus_edit.htm";
...
$skeys = "";
$savedKeys = array();
$keys["GurumaharajId"] = urldecode(postvalue("editid1"));
$savedKeys["GurumaharajId"] = urldecode(postvalue("editid1"));
...
//	processing GurumaharajId - begin
	$condition = 1;

	if($condition)
	{
		$control_GurumaharajId = $pageObject->getControl("GurumaharajId", $id);
		$control_GurumaharajId->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		//	update key value
		if($control_GurumaharajId->getWebValue()!==false)
			$keys["GurumaharajId"] = $control_GurumaharajId->getWebValue();
	}
//	processing GurumaharajId - end
	...
	//	do event
		...
		{		
			...
			//	processing GurumaharajId - begin
				...
				if($condition)
				{
					$control_GurumaharajId->afterSuccessfulSave();
				}
	//	processing GurumaharajId - end
			//	processing Name - begin
							$condition = 1;
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// key get query
	$keyGetQ = "";
		$keyGetQ.="editid1=".rawurldecode($keys["GurumaharajId"])."&";
	// cut last &
	$keyGetQ = substr($keyGetQ, 0, strlen($keyGetQ)-1);	
	// redirect
	header("Location: lu_initiating_gurus_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: lu_initiating_gurus_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
if($readevalues)
{
	$data["GurumaharajId"] = $evalues["GurumaharajId"];
...
if($inlineedit != EDIT_INLINE)
{
	...
	if(!$pageObject->isAppearOnTabs("GurumaharajId"))
		$xt->assign("GurumaharajId_fieldblock",true);
	else
		$xt->assign("GurumaharajId_tabfieldblock",true);
	$xt->assign("GurumaharajId_label",true);
	if(isEnableSection508())
		$xt->assign_section("GurumaharajId_label","<label for=\"".GetInputElementId("GurumaharajId", $id, PAGE_EDIT)."\">","</label>");
	...
	$xt->assign("show_key1", htmlspecialchars($pageObject->showDBValue("GurumaharajId", $data)));
...
//	detail tables
	...
	$keylink = "";
	$keylink.= "&key1=".htmlspecialchars(rawurlencode(@$data["GurumaharajId"]));


//	GurumaharajId - 
	$value = $pageObject->showDBValue("GurumaharajId", $data, $keylink);
	$showValues["GurumaharajId"] = $value;
	$showFields[] = "GurumaharajId";
		$showRawValues["GurumaharajId"] = substr($data["GurumaharajId"],0,100);
...
if($pageObject->isShowDetailTables && $inlineedit!=EDIT_INLINE && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "lu_initiating_gurus";
		$options['firstTime'] = 1;
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='lu_initiating_gurus_view.php?".$viewlink."'\"");
