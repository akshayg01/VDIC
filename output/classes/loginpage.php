...
class LoginPage extends RunnerPage 
{
	...
	*/
	function LogIn($pUsername,$pPassword){
		...
		$fieldList = "";
		$lSet = new ProjectSettings("devotee", PAGE_LIST);
		if($lSet->GetTableData(".sqlquery"))
			$fieldList = $lSet->GetTableData(".sqlquery")->toSql();
		if($fieldList)
		{
			if(!$this->pSet->isCaseInsensitiveUsername()) {
				$where = GetFullFieldName($cUserNameField,"devotee",false).
				   "=".$strUsername." and ".GetFullFieldName($cPasswordField,"devotee",false)."=".$strPassword;
   			} else {
				$where = db_upper(getFullFieldName($cUserNameField,"devotee",false)).
					   "=".$this->pSet->getCaseSensitiveUsername($strUsername)." and ".GetFullFieldName($cPasswordField,"devotee",false).
					   "=".$strPassword;
			}		   
		...
		{
			$strSQL = "select * from `devotee` where ".AddFieldWrappers($cUserNameField)."=".$strUsername." and ".AddFieldWrappers($cPasswordField)."=".$strPassword;
		}
