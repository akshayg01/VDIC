...
{	
	...
	*/
	...
	{	
		...
		$userheaderlink_attrs = "href=\"".$this->tName."_list.php?orderby=AEmailPrimary\"";
		
		//for display name 
		$displayuserheaderlink_attrs = "href=\"".$this->tName."_list.php?orderby="."AFirstName\"";
		
		if ($this->sortByGroup === false && $this->sortField != ""){
			if ($this->sortField == "EmailPrimary"){
				...
				if($this->sortOrder == "A") {
					...
					$userheaderlink_attrs.="DEmailPrimary";
				} else {
					if($this->is508)
					...
					$userheaderlink_attrs.="AEmailPrimary";
				}
				...
				if($this->sortOrder == "A") {
					...
					$displayuserheaderlink_attrs.="DFirstName";
				} else {
					if($this->is508)
					...
					$displayuserheaderlink_attrs.="AFirstName";
				}
	...
	function save() 
	{
		if(postvalue("a")=="save")
		{
			...
			foreach(postvalue("username") as $user)
			{
				...
				$sql="delete from `access1_ugmembers` where UserName=".db_prepare_string($user);
				...
				if(count($_POST["cb_".$useridx]))
				{
					...
					foreach(@$_POST["cb_".$useridx] as $g)
					{
						if($g<0)
						{
							if($user==$_SESSION["UserID"] && $g==-1)
								continue;
							$sql="insert into `access1_ugmembers` (UserName,GroupID) values (".db_prepare_string(htmlspecialchars($user)).",".$g.")";
							db_exec($sql,$this->conn);
						}
						else
						{
					...
					if($glist!="")
					{
						$sql="insert into `access1_ugmembers` (UserName,GroupID) select ".db_prepare_string(htmlspecialchars($user)).",GroupID from `access1_uggroups` where GroupID in (".$glist.")";
						db_exec($sql,$this->conn);
					}
				}
			}
	...
	function fillGridData() 
	{
		...
		while($data && ($this->sortByGroup !== false || $this->recNo <= $this->pageSize || $this->pageSize==-1))
		{
			...
			foreach($this->groups as $idx => $g)
			{
				...
				if($_SESSION["UserID"] != $data["EmailPrimary"] || $g[0] != -1)
				{
			...
			//$row["begin"] = "<input type=hidden name=\"username[]\" value=\"".htmlspecialchars($data["EmailPrimary"])."\">";
			
			$row["username"] = htmlspecialchars($data["EmailPrimary"]);
			$row["displayusername"] = htmlspecialchars($data["FirstName"]); //htmlspecialchars($data["EmailPrimary"]);
			$row["user"] = $data["EmailPrimary"];
			$usercheckbox_attrs.=" name=\"user_".count($this->users)
				."\" id=\"".count($this->users)."\"";
			
			$this->users[] = htmlspecialchars($data["EmailPrimary"]);
	...
	{
		//	select members list		
		$trs = db_query("select `UserName`,`GroupID` from `access1_ugmembers` order by `UserName`,`GroupID`",$this->conn);
	...
	function fillGroups() 
	{
		...
		$trs = db_query("select `GroupID`,`Label` from `access1_uggroups` order by `Label`",$this->conn);
		while($tdata = db_fetch_numarray($trs))
		{
	...
	function setSessionVariables() 
	{
		...
		if (substr($_SESSION[$this->sessionPrefix."_orderby"],1) == "EmailPrimary" || substr($_SESSION[$this->sessionPrefix."_orderby"],1) =="FirstName"){
