...
$layout->blocks["top"][] = "details";$page_layouts["lu_request_types_add"] = $layout;
...
if($inlineadd == ADD_INLINE)
	$templatefile = "lu_request_types_inline_add.htm";
else
	$templatefile = "lu_request_types_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: lu_request_types_add.php');
...
if(@$_POST["a"]=="added")
{
	...
	$avalues=array();
	$blobfields=array();
//	processing RequestTypeId - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_RequestTypeId = $pageObject->getControl("RequestTypeId", $id);
		$control_RequestTypeId->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing RequestTypeId - end
//	processing RequestType - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_RequestType = $pageObject->getControl("RequestType", $id);
		$control_RequestType->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'lu_request_types_edit.php?'.$keylink.'\'>'."Edit".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'lu_request_types_view.php?'.$keylink.'\'>'."View".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: lu_request_types_".$pageObject->getPageType().".php");
...
if(array_key_exists("copyid1",$_REQUEST) || array_key_exists("editid1",$_REQUEST))
{
	...
	{
		$copykeys["RequestTypeId"]=postvalue("copyid1");
	}
	else
	{
		$copykeys["RequestTypeId"]=postvalue("editid1");
	}
...
if($readavalues)
{
	$defvalues["RequestTypeId"]=@$avalues["RequestTypeId"];
	$defvalues["RequestType"]=@$avalues["RequestType"];
}
...
if($inlineadd!=ADD_INLINE)
{
	...
	if(isEnableSection508())
		$xt->assign_section("RequestTypeId_label","<label for=\"".GetInputElementId("RequestTypeId", $id, PAGE_ADD)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("RequestType_label","<label for=\"".GetInputElementId("RequestType", $id, PAGE_ADD)."\">","</label>");
