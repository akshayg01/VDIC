...
// keys array
$keys[]=AddFieldWrappers("DevoteeId");
$keys_present=1;
	...
	for($k=0; $k<count($keys); $k++)
	{
		if (!in_array(RemoveFieldWrappers($keys[$k]),$fields))
		{
			...
			break;
																																	...
																																	if(in_array("organization_OrgId",$fields))
		$autoinc=true;
						if(in_array("spirituallife_SpiritulLifeId",$fields))
		$autoinc=true;
															if(in_array("devoteelang_DevoteeLanguageId",$fields))
		$autoinc=true;
								if(in_array("addresstypes_AddressTypeId",$fields))
		$autoinc=true;
		if(in_array("devoteeaddress_devoteeaddressId",$fields))
		$autoinc=true;
		if(in_array("Address_AddressId",$fields))
		$autoinc=true;
		if(in_array("occupational_DevoteeOccupationalId",$fields))
		$autoinc=true;
		if(in_array("DevoteeOrg_DevoteeOrgId",$fields))
		$autoinc=true;
						if(in_array("DevoteeId",$fields))
		$autoinc=true;
		if(in_array("company_CompanyId",$fields))
		$autoinc=true;
...
$layout = new TLayout("import2","RoundedAqua1","MobileAqua1");
...
$layout->blocks["top"][] = "import";$page_layouts["vw_Devotee_full_import"] = $layout;
...
$xt->display("vw_Devotee_full_import.htm");
