...
add_nocache_headers();
include("include/lu_languages_variables.php");
...
$layout->blocks["top"][] = "details";$page_layouts["lu_languages_add"] = $layout;
...
if($inlineadd == ADD_INLINE)
	$templatefile = "lu_languages_inline_add.htm";
else
	$templatefile = "lu_languages_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: lu_languages_add.php');
...
//	processing language - start
	...
	if($inlineAddOption)
	{
		$control_language = $pageObject->getControl("language", $id);
		$control_language->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing language - end
...
//	processing language - start
			...
			if($inlineAddOption)
			{
				$control_language->afterSuccessfulSave();
			}
//	processing language - end
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'lu_languages_edit.php?'.$keylink.'\'>'."Edit".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'lu_languages_view.php?'.$keylink.'\'>'."View".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: lu_languages_".$pageObject->getPageType().".php");
...
if(array_key_exists("copyid1",$_REQUEST) || array_key_exists("editid1",$_REQUEST))
{
	...
	{
		$copykeys["LanguageId"]=postvalue("copyid1");
	}
	else
	{
		$copykeys["LanguageId"]=postvalue("editid1");
	}
...
if($readavalues)
{
	$defvalues["language"]=@$avalues["language"];
}
...
if($inlineadd!=ADD_INLINE)
{
	...
	if(!$pageObject->isAppearOnTabs("language"))
		$xt->assign("language_fieldblock",true);
	else
		$xt->assign("language_tabfieldblock",true);
	$xt->assign("language_label",true);
	if(isEnableSection508())
		$xt->assign_section("language_label","<label for=\"".GetInputElementId("language", $id, PAGE_ADD)."\">","</label>");
...
if(@$_POST["a"]=="added" && ($inlineadd == ADD_INLINE || $inlineadd == ADD_MASTER || $inlineadd==ADD_POPUP)) 
{
	...
	$keylink="";
	$keylink.="&key1=".htmlspecialchars(rawurlencode(@$data["LanguageId"]));
...
//	language
	...
	if($display)
	{	
		$value = $pageObject->showDBValue("language", $data, $keylink);
		$showValues["language"] = $value;
		$showFields[] = "language";
	}	
		$showRawValues["LanguageId"] = substr($data["LanguageId"],0,100);
		$showRawValues["language"] = substr($data["language"],0,100);
...
if($pageObject->isShowDetailTables && ($inlineadd==ADD_SIMPLE || $inlineadd==ADD_POPUP) && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "lu_languages";
		$options['firstTime'] = 1;
...
function GetAddedDataLookupQuery($pageObject, $keys, $forLookup)
{
	...
	if(postvalue("table")=="devotee_language" && postvalue("field")=="LanguageId") 
	{
		$lookupFieldName = "LanguageId";
		$lookupTable = "lu_languages";
		$linkFieldName = "LanguageId";
		$dispfield = "language";
		// Table which contain lookup field!
		$lookupPSet = new ProjectSettings("devotee_language", PAGE_ADD);
		$nLookupType = $lookupPSet->getLookupType($lookupFieldName);
		if($nLookupType == LT_QUERY)
		{
			if($lookupPSet->getCustomDisplay($lookupFieldName))
				$pageObject->pSet->getSQLQuery()->AddCustomExpression($dispfield, $pageObject->pSet, "devotee_language", "LanguageId");
			...
			$LookupSQL = "select ";
			$LookupSQL .= GetFullFieldName("LanguageId", $strTableName, true);
