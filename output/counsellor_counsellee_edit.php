...
$layout->blocks["top"][] = "details";$page_layouts["counsellor_counsellee_edit"] = $layout;
...
$templatefile = ($inlineedit == EDIT_INLINE) ? "counsellor_counsellee_inline_edit.htm" : "counsellor_counsellee_edit.htm";
...
$skeys = "";
$savedKeys = array();
$keys["CounsellorCounselleeID"] = urldecode(postvalue("editid1"));
$savedKeys["CounsellorCounselleeID"] = urldecode(postvalue("editid1"));
...
//	processing CounselleeDevoteeID - begin
	$condition = 1;

	if($condition)
	{
		$control_CounselleeDevoteeID = $pageObject->getControl("CounselleeDevoteeID", $id);
		$control_CounselleeDevoteeID->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// key get query
	$keyGetQ = "";
		$keyGetQ.="editid1=".rawurldecode($keys["CounsellorCounselleeID"])."&";
	// cut last &
	$keyGetQ = substr($keyGetQ, 0, strlen($keyGetQ)-1);	
	// redirect
	header("Location: counsellor_counsellee_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: counsellor_counsellee_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
if($readevalues)
{
	...
	$data["CounselleeDevoteeID"] = $evalues["CounselleeDevoteeID"];
}
...
if($inlineedit != EDIT_INLINE)
{
	...
	if(isEnableSection508())
		$xt->assign_section("CounselleeDevoteeID_label","<label for=\"".GetInputElementId("CounselleeDevoteeID", $id, PAGE_EDIT)."\">","</label>");
		

	$xt->assign("show_key1", htmlspecialchars($pageObject->showDBValue("CounsellorCounselleeID", $data)));
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='counsellor_counsellee_view.php?".$viewlink."'\"");
