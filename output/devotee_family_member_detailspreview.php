...
$layout->blocks["bare"][] = "detailspreviewgrid";$page_layouts["devotee_family_member_detailspreview"] = $layout;
...
{
	$where = "";
		$where .= GetFullFieldName("DevoteeIdFamilyOwner", $strTableName, false)."=".make_db_value("DevoteeIdFamilyOwner",$_SESSION[$strTableName."_masterkey1"]);
}
...
if($rowcount) {
	...
	//	MemberName - 
		    ...
		    $value = $viewContainer->showDBValue("MemberName", $data, $keylink);
			$row["MemberName_value"] = $value;
	//	MemberRelationshipId - 
			$viewContainer->recId = $recordsCounter;
		    $value = $viewContainer->showDBValue("MemberRelationshipId", $data, $keylink);
			$row["MemberRelationshipId_value"] = $value;
	//	DateofBirth - Short Date
			$viewContainer->recId = $recordsCounter;
		    $value = $viewContainer->showDBValue("DateofBirth", $data, $keylink);
			$row["DateofBirth_value"] = $value;
	//	Remark - 
			$viewContainer->recId = $recordsCounter;
...
$xt->load_template("devotee_family_member_detailspreview.htm");
