...
$layout->blocks["top"][] = "details";$page_layouts["lu_donation_purpose_add"] = $layout;
...
if($inlineadd == ADD_INLINE)
	$templatefile = "lu_donation_purpose_inline_add.htm";
else
	$templatefile = "lu_donation_purpose_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: lu_donation_purpose_add.php');
...
//	processing PurposeTitle - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_PurposeTitle = $pageObject->getControl("PurposeTitle", $id);
		$control_PurposeTitle->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'lu_donation_purpose_edit.php?'.$keylink.'\'>'."Edit".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'lu_donation_purpose_view.php?'.$keylink.'\'>'."View".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: lu_donation_purpose_".$pageObject->getPageType().".php");
...
if(array_key_exists("copyid1",$_REQUEST) || array_key_exists("editid1",$_REQUEST))
{
	...
	{
		$copykeys["DonationPurposeId"]=postvalue("copyid1");
	}
	else
	{
		$copykeys["DonationPurposeId"]=postvalue("editid1");
	}
...
if($readavalues)
{
	...
	$defvalues["PurposeTitle"]=@$avalues["PurposeTitle"];
}
...
if($inlineadd!=ADD_INLINE)
{
	...
	if(isEnableSection508())
		$xt->assign_section("PurposeTitle_label","<label for=\"".GetInputElementId("PurposeTitle", $id, PAGE_ADD)."\">","</label>");
