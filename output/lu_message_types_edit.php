...
include("include/lu_message_types_variables.php");
...
$layout->blocks["top"][] = "details";$page_layouts["lu_message_types_edit"] = $layout;
...
$templatefile = ($inlineedit == EDIT_INLINE) ? "lu_message_types_inline_edit.htm" : "lu_message_types_edit.htm";
...
$skeys = "";
$savedKeys = array();
$keys["MessageTypeId"] = urldecode(postvalue("editid1"));
$savedKeys["MessageTypeId"] = urldecode(postvalue("editid1"));
...
//	processing MessageTypeId - begin
	$condition = 1;

	if($condition)
	{
		$control_MessageTypeId = $pageObject->getControl("MessageTypeId", $id);
		$control_MessageTypeId->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		//	update key value
		if($control_MessageTypeId->getWebValue()!==false)
			$keys["MessageTypeId"] = $control_MessageTypeId->getWebValue();
	}
//	processing MessageTypeId - end
//	processing MessageType - begin
	$condition = 1;

	if($condition)
	{
		$control_MessageType = $pageObject->getControl("MessageType", $id);
		$control_MessageType->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing MessageType - end
	...
	//	do event
		...
		{		
			...
			//	processing MessageTypeId - begin
				...
				if($condition)
				{
					$control_MessageTypeId->afterSuccessfulSave();
				}
	//	processing MessageTypeId - end
			//	processing MessageType - begin
							$condition = 1;
				...
				if($condition)
				{
					$control_MessageType->afterSuccessfulSave();
				}
	//	processing MessageType - end
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// key get query
	$keyGetQ = "";
		$keyGetQ.="editid1=".rawurldecode($keys["MessageTypeId"])."&";
	// cut last &
	$keyGetQ = substr($keyGetQ, 0, strlen($keyGetQ)-1);	
	// redirect
	header("Location: lu_message_types_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: lu_message_types_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
if($readevalues)
{
	$data["MessageTypeId"] = $evalues["MessageTypeId"];
	$data["MessageType"] = $evalues["MessageType"];
}
...
if($inlineedit != EDIT_INLINE)
{
	...
	if(!$pageObject->isAppearOnTabs("MessageTypeId"))
		$xt->assign("MessageTypeId_fieldblock",true);
	else
		$xt->assign("MessageTypeId_tabfieldblock",true);
	$xt->assign("MessageTypeId_label",true);
	if(isEnableSection508())
		$xt->assign_section("MessageTypeId_label","<label for=\"".GetInputElementId("MessageTypeId", $id, PAGE_EDIT)."\">","</label>");
		
	if(!$pageObject->isAppearOnTabs("MessageType"))
		$xt->assign("MessageType_fieldblock",true);
	else
		$xt->assign("MessageType_tabfieldblock",true);
	$xt->assign("MessageType_label",true);
	if(isEnableSection508())
		$xt->assign_section("MessageType_label","<label for=\"".GetInputElementId("MessageType", $id, PAGE_EDIT)."\">","</label>");
		

	$xt->assign("show_key1", htmlspecialchars($pageObject->showDBValue("MessageTypeId", $data)));
...
//	detail tables
	...
	$keylink = "";
	$keylink.= "&key1=".htmlspecialchars(rawurlencode(@$data["MessageTypeId"]));


//	MessageTypeId - 
	$value = $pageObject->showDBValue("MessageTypeId", $data, $keylink);
	$showValues["MessageTypeId"] = $value;
	$showFields[] = "MessageTypeId";
		$showRawValues["MessageTypeId"] = substr($data["MessageTypeId"],0,100);

//	MessageType - 
	$value = $pageObject->showDBValue("MessageType", $data, $keylink);
	$showValues["MessageType"] = $value;
	$showFields[] = "MessageType";
		$showRawValues["MessageType"] = substr($data["MessageType"],0,100);
/////////////////////////////////////////////////////////////
//	start inline output
/////////////////////////////////////////////////////////////
...
if($pageObject->isShowDetailTables && $inlineedit!=EDIT_INLINE && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "lu_message_types";
		$options['firstTime'] = 1;
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='lu_message_types_view.php?".$viewlink."'\"");
