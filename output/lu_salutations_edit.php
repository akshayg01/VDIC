...
$layout->blocks["top"][] = "details";$page_layouts["lu_salutations_edit"] = $layout;
...
$templatefile = ($inlineedit == EDIT_INLINE) ? "lu_salutations_inline_edit.htm" : "lu_salutations_edit.htm";
...
$skeys = "";
$savedKeys = array();
$keys["SalutationId"] = urldecode(postvalue("editid1"));
$savedKeys["SalutationId"] = urldecode(postvalue("editid1"));
...
//	processing salutation - begin
	...
	if($condition)
	{
		...
		$control_salutation->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// key get query
	$keyGetQ = "";
		$keyGetQ.="editid1=".rawurldecode($keys["SalutationId"])."&";
	// cut last &
	$keyGetQ = substr($keyGetQ, 0, strlen($keyGetQ)-1);	
	// redirect
	header("Location: lu_salutations_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: lu_salutations_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
if($inlineedit != EDIT_INLINE)
{
	...
	if(isEnableSection508())
		$xt->assign_section("salutation_label","<label for=\"".GetInputElementId("salutation", $id, PAGE_EDIT)."\">","</label>");
	...
	$xt->assign("show_key1", htmlspecialchars($pageObject->showDBValue("SalutationId", $data)));
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='lu_salutations_view.php?".$viewlink."'\"");
