...
add_nocache_headers();
include("include/lu_salutations_variables.php");
...
$layout->blocks["top"][] = "details";$page_layouts["lu_salutations_add"] = $layout;
...
if($inlineadd == ADD_INLINE)
	$templatefile = "lu_salutations_inline_add.htm";
else
	$templatefile = "lu_salutations_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: lu_salutations_add.php');
...
if(@$_POST["a"]=="added")
{
	...
	$avalues=array();
	$blobfields=array();
//	processing salutation - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_salutation = $pageObject->getControl("salutation", $id);
		$control_salutation->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
...
//	processing salutation - start
			...
			if($inlineAddOption)
			{
				$control_salutation->afterSuccessfulSave();
			}
//	processing salutation - end
//	processing Gender - start
			$inlineAddOption = true;
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'lu_salutations_edit.php?'.$keylink.'\'>'."Edit".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'lu_salutations_view.php?'.$keylink.'\'>'."View".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: lu_salutations_".$pageObject->getPageType().".php");
...
if(array_key_exists("copyid1",$_REQUEST) || array_key_exists("editid1",$_REQUEST))
{
	...
	{
		$copykeys["SalutationId"]=postvalue("copyid1");
	}
	else
	{
		$copykeys["SalutationId"]=postvalue("editid1");
	}
...
if($readavalues)
{
	$defvalues["salutation"]=@$avalues["salutation"];
...
if($inlineadd!=ADD_INLINE)
{
	...
	if(isEnableSection508())
		$xt->assign_section("salutation_label","<label for=\"".GetInputElementId("salutation", $id, PAGE_ADD)."\">","</label>");
...
if(@$_POST["a"]=="added" && ($inlineadd == ADD_INLINE || $inlineadd == ADD_MASTER || $inlineadd==ADD_POPUP)) 
{
	...
	$keylink="";
	$keylink.="&key1=".htmlspecialchars(rawurlencode(@$data["SalutationId"]));
...
//	SalutationId
	...
	if($display)
	{	
		$value = $pageObject->showDBValue("SalutationId", $data, $keylink);
		$showValues["SalutationId"] = $value;
		$showFields[] = "SalutationId";
	}	
//	salutation
	$display = false;
	...
	if($display)
	{	
		$value = $pageObject->showDBValue("salutation", $data, $keylink);
...
//	Gender
	...
	{	
		...
		$showRawValues["SalutationId"] = substr($data["SalutationId"],0,100);
...
if($pageObject->isShowDetailTables && ($inlineadd==ADD_SIMPLE || $inlineadd==ADD_POPUP) && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "lu_salutations";
		$options['firstTime'] = 1;
...
function GetAddedDataLookupQuery($pageObject, $keys, $forLookup)
{
	...
	if(postvalue("table")=="devotee" && postvalue("field")=="TitleId") 
	{
		$lookupFieldName = "TitleId";
		$lookupTable = "lu_salutations";
		$linkFieldName = "SalutationId";
		$dispfield = "salutation";
		// Table which contain lookup field!
		$lookupPSet = new ProjectSettings("devotee", PAGE_ADD);
		$nLookupType = $lookupPSet->getLookupType($lookupFieldName);
		if($nLookupType == LT_QUERY)
		{
			if($lookupPSet->getCustomDisplay($lookupFieldName))
				$pageObject->pSet->getSQLQuery()->AddCustomExpression($dispfield, $pageObject->pSet, "devotee", "TitleId");
			...
			$LookupSQL = "select ";
			$LookupSQL .= GetFullFieldName("SalutationId", $strTableName, true);
					if($linkFieldName != $dispfield)
				$LookupSQL .= ",salutation";
		}
	}
	if(postvalue("table")=="vw_counsellee" && postvalue("field")=="TitleId") 
	{
		$lookupFieldName = "TitleId";
		$lookupTable = "lu_salutations";
		$linkFieldName = "SalutationId";
		$dispfield = "salutation";
		// Table which contain lookup field!
		$lookupPSet = new ProjectSettings("vw_counsellee", PAGE_ADD);
		$nLookupType = $lookupPSet->getLookupType($lookupFieldName);
		if($nLookupType == LT_QUERY)
		{
			if($lookupPSet->getCustomDisplay($lookupFieldName))
				$pageObject->pSet->getSQLQuery()->AddCustomExpression($dispfield, $pageObject->pSet, "vw_counsellee", "TitleId");
			$lookupQueryObj = $pageObject->pSet->getSQLQuery()->CloneObject();
			if(!$noBlobReplace)
				$lookupQueryObj->ReplaceFieldsWithDummies($pageObject->pSet->getBinaryFieldsIndices());
		}else{
	...
	if(postvalue("table")=="vw_OwnProfile" && postvalue("field")=="TitleId") 
	{
		$lookupFieldName = "TitleId";
		$lookupTable = "lu_salutations";
		$linkFieldName = "SalutationId";
		$dispfield = "salutation";
		// Table which contain lookup field!
		$lookupPSet = new ProjectSettings("vw_OwnProfile", PAGE_ADD);
		$nLookupType = $lookupPSet->getLookupType($lookupFieldName);
		if($nLookupType == LT_QUERY)
		{
			if($lookupPSet->getCustomDisplay($lookupFieldName))
				$pageObject->pSet->getSQLQuery()->AddCustomExpression($dispfield, $pageObject->pSet, "vw_OwnProfile", "TitleId");
			$lookupQueryObj = $pageObject->pSet->getSQLQuery()->CloneObject();
			if(!$noBlobReplace)
				$lookupQueryObj->ReplaceFieldsWithDummies($pageObject->pSet->getBinaryFieldsIndices());
		}else{
	...
	if(postvalue("table")=="vw_Devotee_full" && postvalue("field")=="TitleId") 
	{
		$lookupFieldName = "TitleId";
		$lookupTable = "lu_salutations";
		$linkFieldName = "SalutationId";
		$dispfield = "salutation";
		// Table which contain lookup field!
		$lookupPSet = new ProjectSettings("vw_Devotee_full", PAGE_ADD);
		$nLookupType = $lookupPSet->getLookupType($lookupFieldName);
		if($nLookupType == LT_QUERY)
		{
			if($lookupPSet->getCustomDisplay($lookupFieldName))
				$pageObject->pSet->getSQLQuery()->AddCustomExpression($dispfield, $pageObject->pSet, "vw_Devotee_full", "TitleId");
			$lookupQueryObj = $pageObject->pSet->getSQLQuery()->CloneObject();
			if(!$noBlobReplace)
				$lookupQueryObj->ReplaceFieldsWithDummies($pageObject->pSet->getBinaryFieldsIndices());
		}else{
	...
	if(postvalue("table")=="vw_counsellee_temp" && postvalue("field")=="TitleId") 
	{
		$lookupFieldName = "TitleId";
		$lookupTable = "lu_salutations";
		$linkFieldName = "SalutationId";
		$dispfield = "salutation";
		// Table which contain lookup field!
		$lookupPSet = new ProjectSettings("vw_counsellee_temp", PAGE_ADD);
		$nLookupType = $lookupPSet->getLookupType($lookupFieldName);
		if($nLookupType == LT_QUERY)
		{
			if($lookupPSet->getCustomDisplay($lookupFieldName))
				$pageObject->pSet->getSQLQuery()->AddCustomExpression($dispfield, $pageObject->pSet, "vw_counsellee_temp", "TitleId");
			$lookupQueryObj = $pageObject->pSet->getSQLQuery()->CloneObject();
			if(!$noBlobReplace)
				$lookupQueryObj->ReplaceFieldsWithDummies($pageObject->pSet->getBinaryFieldsIndices());
		}else{
