...
$layout->blocks["top"][] = "export";$page_layouts["lu_donation_purpose_export"] = $layout;
...
$xt->display("lu_donation_purpose_export.htm");

function ExportToExcel_old($cipherer)
{
	global $cCharset;
	header("Content-Type: application/vnd.ms-excel");
	header("Content-Disposition: attachment;Filename=lu_donation_purpose.xls");
...
function ExportToWord($cipherer)
{
	...
	header("Content-Disposition: attachment;Filename=lu_donation_purpose.doc");
...
function ExportToXML($cipherer)
{
	...
	header("Content-Disposition: attachment;Filename=lu_donation_purpose.xml");
	...
	while((!$nPageSize || $i<$nPageSize) && $row)
	{
		$values = array();
			...
			$values["PurposeTitle"] = $pageObject->showDBValue("PurposeTitle", $row);
...
function ExportToCSV($cipherer)
{
	...
	header("Content-Disposition: attachment;Filename=lu_donation_purpose.csv");
...
// write data rows
	...
	while((!$nPageSize || $iNumberOfRows < $nPageSize) && $row)
	{
		$values = array();
			...
			$values["PurposeTitle"] = $pageObject->getViewControl("PurposeTitle")->showDBValue($row, "");
		...
		if ($eventRes)
		{
			...
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["PurposeTitle"]).'"';
			echo $outstr;
		}
...
// write header
	...
	if($_REQUEST["type"]=="excel")
	{
		...
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Purpose Title").'</td>';	
	}
	else
	{
		echo "<td>"."Donation Purpose Id"."</td>";
		echo "<td>"."Purpose Title"."</td>";
	}
	echo "</tr>";
		...
		if ($eventRes)
		{
			...
			echo '</td>';
					...
					echo '<td>';
									...
									if($_REQUEST["type"]=="excel")
						echo PrepareForExcel($values["PurposeTitle"]);
					else
						echo $values["PurposeTitle"];
			echo '</td>';
			echo "</tr>";
		}
