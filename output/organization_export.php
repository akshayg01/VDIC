...
$layout->blocks["top"][] = "export";$page_layouts["organization_export"] = $layout;
...
$xt->display("organization_export.htm");
...
function ExportToWord($cipherer)
{
	...
	header("Content-Disposition: attachment;Filename=organization.doc");
...
function ExportToXML($cipherer)
{
	...
	header("Content-Disposition: attachment;Filename=organization.xml");
	...
	while((!$nPageSize || $i<$nPageSize) && $row)
	{
		$values = array();
			...
			$values["OrgName"] = $pageObject->showDBValue("OrgName", $row);
			$values["OrgOwnerId"] = $pageObject->showDBValue("OrgOwnerId", $row);
...
// write data rows
	...
	while((!$nPageSize || $iNumberOfRows < $nPageSize) && $row)
	{
		$values = array();
			...
			$values["OrgName"] = $pageObject->getViewControl("OrgName")->showDBValue($row, "");
			$values["OrgOwnerId"] = $pageObject->getViewControl("OrgOwnerId")->showDBValue($row, "");
		...
		if ($eventRes)
		{
			...
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["OrgName"]).'"';
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["OrgOwnerId"]).'"';
			if($outstr!="")
				$outstr.=",";
...
// write header
	...
	if($_REQUEST["type"]=="excel")
	{
		...
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Org Owner Id").'</td>';	
		...
		if ($eventRes)
		{
			...
			echo '</td>';
					...
					echo '<td>';
									...
									if($_REQUEST["type"]=="excel")
						echo PrepareForExcel($values["OrgName"]);
					else
						echo $values["OrgName"];
			echo '</td>';
							echo '<td>';
								...
								if($_REQUEST["type"]=="excel")
					echo PrepareForExcel($values["OrgOwnerId"]);
				else
					echo $values["OrgOwnerId"];//echo htmlspecialchars($values["OrgOwnerId"]); commented for bug #6823
