...
$layout->blocks["top"][] = "export";$page_layouts["devotee_family_member_export"] = $layout;
...
$xt->display("devotee_family_member_export.htm");

function ExportToExcel_old($cipherer)
{
	global $cCharset;
	header("Content-Type: application/vnd.ms-excel");
	header("Content-Disposition: attachment;Filename=devotee_family_member.xls");
...
function ExportToWord($cipherer)
{
	...
	header("Content-Disposition: attachment;Filename=devotee_family_member.doc");
...
function ExportToXML($cipherer)
{
	...
	header("Content-Disposition: attachment;Filename=devotee_family_member.xml");
	...
	{
		$values = array();
			$values["MemberName"] = $pageObject->showDBValue("MemberName", $row);
			$values["MemberRelationshipId"] = $pageObject->showDBValue("MemberRelationshipId", $row);
...
function ExportToCSV($cipherer)
{
	...
	header("Content-Disposition: attachment;Filename=devotee_family_member.csv");
...
// write data rows
	...
	{
		$values = array();
			$values["MemberName"] = $pageObject->getViewControl("MemberName")->showDBValue($row, "");
			$values["MemberRelationshipId"] = $pageObject->getViewControl("MemberRelationshipId")->showDBValue($row, "");
		...
		if ($eventRes)
		{
			...
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["MemberName"]).'"';
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["MemberRelationshipId"]).'"';
			if($outstr!="")
				$outstr.=",";
...
// write header
	...
	{
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Member Name").'</td>';	
		...
		if ($eventRes)
		{
			...
			echo "<tr>";
					...
					echo '<td>';
								...
								if($_REQUEST["type"]=="excel")
					echo PrepareForExcel($values["MemberName"]);
				else
					echo $values["MemberName"];//echo htmlspecialchars($values["MemberName"]); commented for bug #6823
			...
			echo '</td>';
							echo '<td>';
								...
								if($_REQUEST["type"]=="excel")
					echo PrepareForExcel($values["MemberRelationshipId"]);
				else
					echo $values["MemberRelationshipId"];//echo htmlspecialchars($values["MemberRelationshipId"]); commented for bug #6823
