...
$layout->blocks["top"][] = "search";$page_layouts["lu_languages_search"] = $layout;
...
$mode = SEARCH_SIMPLE;
$templatefile = "lu_languages_search.htm";
...
$params['shortTableName'] = 'lu_languages';
...
if ($mode==SEARCH_SIMPLE)
{
	...
	if($pageObject->pSet->getLookupTable("language"))
		$pageObject->settingsMap["globalSettings"]['shortTNames'][$pageObject->pSet->getLookupTable("language")] = GetTableURL($pageObject->pSet->getLookupTable("language"));
	
	$pageObject->fillFieldToolTips("language");	
	
	$srchFields = $pageObject->searchClauseObj->getSearchCtrlParams("language");
	$firstFieldParams = array();
	if (count($srchFields))
	{
		$firstFieldParams = $srchFields[0];
	...
	if(isEnableSection508())
		$xt->assign_section("language_label","<label for=\"".GetInputElementId("language", $id, PAGE_SEARCH)."\">","</label>");
	else 
	...
	$isFieldNeedSecCtrl = $searchControlBuilder->isNeedSecondCtrl("language");
	$ctrlInd = 0;
	if ($isFieldNeedSecCtrl)
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"language", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd, 1=>($ctrlInd+1)));
		$ctrlInd+=2;
	}
	else
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"language", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd));			
		$ctrlInd++;
	}
