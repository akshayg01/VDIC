...
$layout->blocks["top"][] = "details";$page_layouts["devotee_family_member_add"] = $layout;
...
if($inlineadd == ADD_INLINE)
	$templatefile = "devotee_family_member_inline_add.htm";
else
	$templatefile = "devotee_family_member_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: devotee_family_member_add.php');
...
if(@$_POST["a"]=="added")
{
	...
	$avalues=array();
	$blobfields=array();
//	processing MemberName - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_MemberName = $pageObject->getControl("MemberName", $id);
		$control_MemberName->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing MemberName - end
//	processing MemberRelationshipId - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_MemberRelationshipId = $pageObject->getControl("MemberRelationshipId", $id);
		$control_MemberRelationshipId->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
...
//	insert masterkey value if exists and if not specified
	if(@$_SESSION[$sessionPrefix."_mastertable"]=="devotee")
	{
		...
		if($avalues["DevoteeIdFamilyOwner"]==""){
			$avalues["DevoteeIdFamilyOwner"] = prepare_for_db("DevoteeIdFamilyOwner",$_SESSION[$sessionPrefix."_masterkey1"]);
		}
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'devotee_family_member_edit.php?'.$keylink.'\'>'."Edit".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'devotee_family_member_view.php?'.$keylink.'\'>'."View".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: devotee_family_member_".$pageObject->getPageType().".php");
...
if(array_key_exists("copyid1",$_REQUEST) || array_key_exists("editid1",$_REQUEST))
{
	...
	{
		$copykeys["FamilyMemberId"]=postvalue("copyid1");
	}
	else
	{
		$copykeys["FamilyMemberId"]=postvalue("editid1");
	}
...
if(@$_SESSION[$sessionPrefix."_mastertable"]=="devotee")
{
	...
	if(postvalue("mainMPageType")<>"add")
		$defvalues["DevoteeIdFamilyOwner"] = @$_SESSION[$sessionPrefix."_masterkey1"];	
	
}

if($readavalues)
{
	$defvalues["MemberName"]=@$avalues["MemberName"];
	$defvalues["MemberRelationshipId"]=@$avalues["MemberRelationshipId"];
...
if($inlineadd!=ADD_INLINE)
{
	...
	if(isEnableSection508())
		$xt->assign_section("MemberName_label","<label for=\"".GetInputElementId("MemberName", $id, PAGE_ADD)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("MemberRelationshipId_label","<label for=\"".GetInputElementId("MemberRelationshipId", $id, PAGE_ADD)."\">","</label>");
