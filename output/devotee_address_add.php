...
$layout->blocks["top"][] = "details";$page_layouts["devotee_address_add"] = $layout;
...
if($inlineadd == ADD_INLINE)
	$templatefile = "devotee_address_inline_add.htm";
else
	$templatefile = "devotee_address_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: devotee_address_add.php');
...
if(@$_POST["a"]=="added")
{
	...
	$avalues=array();
	$blobfields=array();
//	processing AddressLine1 - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_AddressLine1 = $pageObject->getControl("AddressLine1", $id);
		$control_AddressLine1->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing AddressLine1 - end
//	processing AddressLine2 - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_AddressLine2 = $pageObject->getControl("AddressLine2", $id);
		$control_AddressLine2->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing AddressLine2 - end
...
//	processing pincode - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_pincode = $pageObject->getControl("pincode", $id);
		$control_pincode->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing pincode - end
//	processing AddressTypeID - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_AddressTypeID = $pageObject->getControl("AddressTypeID", $id);
		$control_AddressTypeID->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing AddressTypeID - end

//	insert ownerid value if exists
	if($strOriginalTableName == $pageObject->pSet->getOwnerTable("AddressId"))
		$avalues["AddressId"] = prepare_for_db("AddressId",$_SESSION["_".$strTableName."_OwnerID"]);
...
//	processing AddressLine1 - start
			...
			if($inlineAddOption)
			{
				$control_AddressLine1->afterSuccessfulSave();
			}
//	processing AddressLine1 - end
//	processing AddressLine2 - start
			$inlineAddOption = true;
			if($inlineAddOption)
			{
				$control_AddressLine2->afterSuccessfulSave();
			}
//	processing AddressLine2 - end
//	processing State - start
			$inlineAddOption = true;
...
//	processing pincode - start
			...
			if($inlineAddOption)
			{
				$control_pincode->afterSuccessfulSave();
			}
//	processing pincode - end
//	processing AddressTypeID - start
			$inlineAddOption = true;
			if($inlineAddOption)
			{
				$control_AddressTypeID->afterSuccessfulSave();
			}
//	processing AddressTypeID - end
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'devotee_address_edit.php?'.$keylink.'\'>'."Edit".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'devotee_address_view.php?'.$keylink.'\'>'."View".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: devotee_address_".$pageObject->getPageType().".php");
...
//	insert default ownerid value if exists
	if($strOriginalTableName == $pageObject->pSet->getOwnerTable("AddressId"))
		$defvalues["AddressId"] = prepare_for_db("AddressId",$_SESSION["_".$strTableName."_OwnerID"]);
...
if($readavalues)
{
	$defvalues["AddressLine1"]=@$avalues["AddressLine1"];
	$defvalues["AddressLine2"]=@$avalues["AddressLine2"];
	$defvalues["City"]=@$avalues["City"];
	$defvalues["State"]=@$avalues["State"];
	$defvalues["country"]=@$avalues["country"];
	$defvalues["pincode"]=@$avalues["pincode"];
	$defvalues["AddressTypeID"]=@$avalues["AddressTypeID"];
}
...
if($inlineadd!=ADD_INLINE)
{
	...
	if(!$pageObject->isAppearOnTabs("AddressLine1"))
		$xt->assign("AddressLine1_fieldblock",true);
	else
		$xt->assign("AddressLine1_tabfieldblock",true);
	$xt->assign("AddressLine1_label",true);
	if(isEnableSection508())
		$xt->assign_section("AddressLine1_label","<label for=\"".GetInputElementId("AddressLine1", $id, PAGE_ADD)."\">","</label>");
	
	if(!$pageObject->isAppearOnTabs("AddressLine2"))
		$xt->assign("AddressLine2_fieldblock",true);
	else
		$xt->assign("AddressLine2_tabfieldblock",true);
	$xt->assign("AddressLine2_label",true);
	if(isEnableSection508())
		$xt->assign_section("AddressLine2_label","<label for=\"".GetInputElementId("AddressLine2", $id, PAGE_ADD)."\">","</label>");
	...
	if(!$pageObject->isAppearOnTabs("pincode"))
		$xt->assign("pincode_fieldblock",true);
	else
		$xt->assign("pincode_tabfieldblock",true);
	$xt->assign("pincode_label",true);
	if(isEnableSection508())
		$xt->assign_section("pincode_label","<label for=\"".GetInputElementId("pincode", $id, PAGE_ADD)."\">","</label>");
	
	if(!$pageObject->isAppearOnTabs("AddressTypeID"))
		$xt->assign("AddressTypeID_fieldblock",true);
	else
		$xt->assign("AddressTypeID_tabfieldblock",true);
	$xt->assign("AddressTypeID_label",true);
	if(isEnableSection508())
		$xt->assign_section("AddressTypeID_label","<label for=\"".GetInputElementId("AddressTypeID", $id, PAGE_ADD)."\">","</label>");
...
//	AddressLine1
	...
	if($display)
	{	
		$value = $pageObject->showDBValue("AddressLine1", $data, $keylink);
		$showValues["AddressLine1"] = $value;
		$showFields[] = "AddressLine1";
	}	
//	AddressLine2
	$display = false;
	if($inlineadd==ADD_MASTER)
	...
	if($display)
	{	
		$value = $pageObject->showDBValue("AddressLine2", $data, $keylink);
		$showValues["AddressLine2"] = $value;
		$showFields[] = "AddressLine2";
	}	
//	City
	$display = false;
	if($inlineadd==ADD_MASTER)
...
//	pincode
	...
	if($display)
	{	
		$value = $pageObject->showDBValue("pincode", $data, $keylink);
		$showValues["pincode"] = $value;
		$showFields[] = "pincode";
	}	
//	AddressTypeID
	$display = false;
	...
	if($display)
	{	
		$value = $pageObject->showDBValue("AddressTypeID", $data, $keylink);
		$showValues["AddressTypeID"] = $value;
		$showFields[] = "AddressTypeID";
	}	
		$showRawValues["AddressLine1"] = substr($data["AddressLine1"],0,100);
		$showRawValues["AddressLine2"] = substr($data["AddressLine2"],0,100);
		...
		$showRawValues["pincode"] = substr($data["pincode"],0,100);
		$showRawValues["IsVerified"] = substr($data["IsVerified"],0,100);
		$showRawValues["IsWrong"] = substr($data["IsWrong"],0,100);
		$showRawValues["AddressTypeID"] = substr($data["AddressTypeID"],0,100);
...
if($pageObject->isShowDetailTables && ($inlineadd==ADD_SIMPLE || $inlineadd==ADD_POPUP) && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "devotee_address";
		$options['firstTime'] = 1;
