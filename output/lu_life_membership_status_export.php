...
$layout->blocks["top"][] = "export";$page_layouts["lu_life_membership_status_export"] = $layout;
...
//	process selection
	...
	if (@$_REQUEST["mdelete"])
	{
		...
		{
			$keys=array();
			$keys["LifeMembershipStatusId"] = refine($_REQUEST["mdelete1"][mdeleteIndex($ind)]);
			$selected_recs[] = $keys;
		}
	}
	elseif(@$_REQUEST["selection"])
	{
		foreach(@$_REQUEST["selection"] as $keyblock)
		{
			...
			if(count($arr)<1)
				continue;
			$keys = array();
			$keys["LifeMembershipStatusId"] = urldecode($arr[0]);
			$selected_recs[] = $keys;
		}
	}
...
$xt->display("lu_life_membership_status_export.htm");

function ExportToExcel_old($cipherer)
{
	global $cCharset;
	header("Content-Type: application/vnd.ms-excel");
	header("Content-Disposition: attachment;Filename=lu_life_membership_status.xls");
...
function ExportToWord($cipherer)
{
	...
	header("Content-Disposition: attachment;Filename=lu_life_membership_status.doc");
...
function ExportToXML($cipherer)
{
	...
	header("Content-Disposition: attachment;Filename=lu_life_membership_status.xml");
	...
	{
		$values = array();
			$values["LifeMembershipStatusId"] = $pageObject->showDBValue("LifeMembershipStatusId", $row);
...
function ExportToCSV($cipherer)
{
	...
	header("Content-Disposition: attachment;Filename=lu_life_membership_status.csv");
...
// write data rows
	...
	{
		$values = array();
			$values["LifeMembershipStatusId"] = $pageObject->getViewControl("LifeMembershipStatusId")->showDBValue($row, "");
		...
		if ($eventRes)
		{
			...
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["LifeMembershipStatusId"]).'"';
			if($outstr!="")
				$outstr.=",";
...
// write header
	...
	{
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Life Membership Status Id").'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Status").'</td>';	
	}
	else
	{
		echo "<td>"."Life Membership Status Id"."</td>";
		echo "<td>"."Status"."</td>";
	}
	echo "</tr>";
		...
		if ($eventRes)
		{
			...
			echo "<tr>";
							...
							echo '<td>';
									...
									echo $values["LifeMembershipStatusId"];
			echo '</td>';
							if($_REQUEST["type"]=="excel")
					echo '<td x:str>';
				else
