...
$layout->blocks["top"][] = "export";$page_layouts["lu_banks_export"] = $layout;
...
//	process selection
	...
	if (@$_REQUEST["mdelete"])
	{
		...
		{
			$keys=array();
			$keys["BankId"] = refine($_REQUEST["mdelete1"][mdeleteIndex($ind)]);
			$selected_recs[] = $keys;
		}
	}
...
$xt->display("lu_banks_export.htm");

function ExportToExcel_old($cipherer)
{
	global $cCharset;
	header("Content-Type: application/vnd.ms-excel");
	header("Content-Disposition: attachment;Filename=lu_banks.xls");
...
function ExportToWord($cipherer)
{
	...
	header("Content-Disposition: attachment;Filename=lu_banks.doc");
...
function ExportToXML($cipherer)
{
	...
	header("Content-Disposition: attachment;Filename=lu_banks.xml");
	...
	{
		$values = array();
			$values["BankId"] = $pageObject->showDBValue("BankId", $row);
...
function ExportToCSV($cipherer)
{
	...
	header("Content-Disposition: attachment;Filename=lu_banks.csv");
...
// write data rows
	...
	{
		$values = array();
			$values["BankId"] = $pageObject->getViewControl("BankId")->showDBValue($row, "");
		...
		if ($eventRes)
		{
			...
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["BankId"]).'"';
			if($outstr!="")
				$outstr.=",";
