...
$layout->blocks["top"][] = "details";$page_layouts["vw_counsellee_add"] = $layout;
...
$layout->blocks["bare"][] = "1";$page_layouts["vw_counsellee_add_Email1"] = $layout;
...
$layout->skins["1"] = "fields";
$layout->blocks["bare"][] = "1";$page_layouts["vw_counsellee_add_Marital1"] = $layout;
...
$layout->skins["1"] = "fields";
$layout->blocks["bare"][] = "1";$page_layouts["vw_counsellee_add_Phone1"] = $layout;
...
$layout->skins["1"] = "fields";
$layout->blocks["bare"][] = "1";$page_layouts["vw_counsellee_add_tab21"] = $layout;
...
$layout->skins["1"] = "fields";
$layout->blocks["bare"][] = "1";$page_layouts["vw_counsellee_add_tab31"] = $layout;
...
if($inlineadd == ADD_INLINE)
	$templatefile = "vw_counsellee_inline_add.htm";
else
	$templatefile = "vw_counsellee_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: vw_counsellee_add.php');
...
if(@$_POST["a"]=="added")
{
	...
	$avalues=array();
	$blobfields=array();
//	processing DevoteeExtraInfoId - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd==ADD_INLINE;
	if($inlineAddOption)
	{
		$control_DevoteeExtraInfoId = $pageObject->getControl("DevoteeExtraInfoId", $id);
		$control_DevoteeExtraInfoId->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing DevoteeExtraInfoId - end
//	processing DevoteeId2 - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd==ADD_INLINE;
	if($inlineAddOption)
	{
		$control_DevoteeId2 = $pageObject->getControl("DevoteeId2", $id);
		$control_DevoteeId2->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing DevoteeId2 - end
//	processing DevoteeLanguageId - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd==ADD_INLINE;
	if($inlineAddOption)
	{
		$control_DevoteeLanguageId = $pageObject->getControl("DevoteeLanguageId", $id);
		$control_DevoteeLanguageId->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing DevoteeLanguageId - end
//	processing DevoteeId1 - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd==ADD_INLINE;
	if($inlineAddOption)
	{
		$control_DevoteeId1 = $pageObject->getControl("DevoteeId1", $id);
		$control_DevoteeId1->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing DevoteeId1 - end
//	processing LanguageId - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd==ADD_INLINE;
	if($inlineAddOption)
	{
		$control_LanguageId = $pageObject->getControl("LanguageId", $id);
		$control_LanguageId->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
...
//	processing SpeakingLevel - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_SpeakingLevel = $pageObject->getControl("SpeakingLevel", $id);
		$control_SpeakingLevel->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing SpeakingLevel - end
//	processing ReadingLevel - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_ReadingLevel = $pageObject->getControl("ReadingLevel", $id);
		$control_ReadingLevel->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing ReadingLevel - end
//	processing WritingLevel - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_WritingLevel = $pageObject->getControl("WritingLevel", $id);
		$control_WritingLevel->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
...
//	processing MotherName - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_MotherName = $pageObject->getControl("MotherName", $id);
		$control_MotherName->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing MotherName - end
//	processing FatherName - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_FatherName = $pageObject->getControl("FatherName", $id);
		$control_FatherName->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
...
//	insert ownerid value if exists
	if($strOriginalTableName == $pageObject->pSet->getOwnerTable("CounsellorDevoteeID"))
		$avalues["CounsellorDevoteeID"] = prepare_for_db("CounsellorDevoteeID",$_SESSION["_".$strTableName."_OwnerID"]);
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'vw_counsellee_edit.php?'.$keylink.'\'>'."Edit".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'vw_counsellee_view.php?'.$keylink.'\'>'."View".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: vw_counsellee_".$pageObject->getPageType().".php");
...
//	insert default ownerid value if exists
	if($strOriginalTableName == $pageObject->pSet->getOwnerTable("CounsellorDevoteeID"))
		$defvalues["CounsellorDevoteeID"] = prepare_for_db("CounsellorDevoteeID",$_SESSION["_".$strTableName."_OwnerID"]);
...
if($readavalues)
{
	...
	$defvalues["FatherName"]=@$avalues["FatherName"];
	$defvalues["MotherName"]=@$avalues["MotherName"];
	$defvalues["SpeakingLevel"]=@$avalues["SpeakingLevel"];
	$defvalues["ReadingLevel"]=@$avalues["ReadingLevel"];
	$defvalues["WritingLevel"]=@$avalues["WritingLevel"];
}
...
if($inlineadd!=ADD_INLINE)
{
	...
	if(!$pageObject->isAppearOnTabs("FatherName"))
		$xt->assign("FatherName_fieldblock",true);
	else
		$xt->assign("FatherName_tabfieldblock",true);
	$xt->assign("FatherName_label",true);
	if(isEnableSection508())
		$xt->assign_section("FatherName_label","<label for=\"".GetInputElementId("FatherName", $id, PAGE_ADD)."\">","</label>");
	
	if(!$pageObject->isAppearOnTabs("MotherName"))
		$xt->assign("MotherName_fieldblock",true);
	else
		$xt->assign("MotherName_tabfieldblock",true);
	$xt->assign("MotherName_label",true);
	if(isEnableSection508())
		$xt->assign_section("MotherName_label","<label for=\"".GetInputElementId("MotherName", $id, PAGE_ADD)."\">","</label>");
	
	if(!$pageObject->isAppearOnTabs("SpeakingLevel"))
		$xt->assign("SpeakingLevel_fieldblock",true);
	else
		$xt->assign("SpeakingLevel_tabfieldblock",true);
	$xt->assign("SpeakingLevel_label",true);
	if(isEnableSection508())
		$xt->assign_section("SpeakingLevel_label","<label for=\"".GetInputElementId("SpeakingLevel", $id, PAGE_ADD)."\">","</label>");
	
	if(!$pageObject->isAppearOnTabs("ReadingLevel"))
		$xt->assign("ReadingLevel_fieldblock",true);
	else
		$xt->assign("ReadingLevel_tabfieldblock",true);
	$xt->assign("ReadingLevel_label",true);
	if(isEnableSection508())
		$xt->assign_section("ReadingLevel_label","<label for=\"".GetInputElementId("ReadingLevel", $id, PAGE_ADD)."\">","</label>");
	
	if(!$pageObject->isAppearOnTabs("WritingLevel"))
		$xt->assign("WritingLevel_fieldblock",true);
	else
		$xt->assign("WritingLevel_tabfieldblock",true);
	$xt->assign("WritingLevel_label",true);
	if(isEnableSection508())
		$xt->assign_section("WritingLevel_label","<label for=\"".GetInputElementId("WritingLevel", $id, PAGE_ADD)."\">","</label>");
