...
$layout->blocks["top"][] = "details";$page_layouts["lu_family_relationship_add"] = $layout;
...
if($inlineadd == ADD_INLINE)
	$templatefile = "lu_family_relationship_inline_add.htm";
else
	$templatefile = "lu_family_relationship_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: lu_family_relationship_add.php');
...
if(@$_POST["a"]=="added")
{
	...
	$avalues=array();
	$blobfields=array();
//	processing RelationshipId - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_RelationshipId = $pageObject->getControl("RelationshipId", $id);
		$control_RelationshipId->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing RelationshipId - end
//	processing RelationshipName - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_RelationshipName = $pageObject->getControl("RelationshipName", $id);
		$control_RelationshipName->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'lu_family_relationship_edit.php?'.$keylink.'\'>'."Edit".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'lu_family_relationship_view.php?'.$keylink.'\'>'."View".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: lu_family_relationship_".$pageObject->getPageType().".php");
...
if(array_key_exists("copyid1",$_REQUEST) || array_key_exists("editid1",$_REQUEST))
{
	...
	{
		$copykeys["RelationshipId"]=postvalue("copyid1");
	}
	else
	{
		$copykeys["RelationshipId"]=postvalue("editid1");
	}
...
if($readavalues)
{
	$defvalues["RelationshipId"]=@$avalues["RelationshipId"];
	$defvalues["RelationshipName"]=@$avalues["RelationshipName"];
}
...
if($inlineadd!=ADD_INLINE)
{
	...
	if(isEnableSection508())
		$xt->assign_section("RelationshipId_label","<label for=\"".GetInputElementId("RelationshipId", $id, PAGE_ADD)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("RelationshipName_label","<label for=\"".GetInputElementId("RelationshipName", $id, PAGE_ADD)."\">","</label>");
