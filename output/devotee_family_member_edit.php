...
include("include/devotee_family_member_variables.php");
...
$layout->blocks["top"][] = "details";$page_layouts["devotee_family_member_edit"] = $layout;
...
$templatefile = ($inlineedit == EDIT_INLINE) ? "devotee_family_member_inline_edit.htm" : "devotee_family_member_edit.htm";
...
$skeys = "";
$savedKeys = array();
$keys["FamilyMemberId"] = urldecode(postvalue("editid1"));
$savedKeys["FamilyMemberId"] = urldecode(postvalue("editid1"));
...
//	processing MemberName - begin
	$condition = 1;

	if($condition)
	{
		$control_MemberName = $pageObject->getControl("MemberName", $id);
		$control_MemberName->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing MemberName - end
//	processing MemberRelationshipId - begin
	$condition = 1;

	if($condition)
	{
		$control_MemberRelationshipId = $pageObject->getControl("MemberRelationshipId", $id);
		$control_MemberRelationshipId->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing MemberRelationshipId - end
	...
	//	do event
		...
		{		
			...
			//	processing MemberName - begin
				...
				if($condition)
				{
					$control_MemberName->afterSuccessfulSave();
				}
	//	processing MemberName - end
			//	processing MemberRelationshipId - begin
							$condition = 1;
				...
				if($condition)
				{
					$control_MemberRelationshipId->afterSuccessfulSave();
				}
	//	processing MemberRelationshipId - end
			//	processing DateofBirth - begin
							$condition = 1;
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// key get query
	$keyGetQ = "";
		$keyGetQ.="editid1=".rawurldecode($keys["FamilyMemberId"])."&";
	// cut last &
	$keyGetQ = substr($keyGetQ, 0, strlen($keyGetQ)-1);	
	// redirect
	header("Location: devotee_family_member_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: devotee_family_member_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
if($readevalues)
{
	$data["MemberName"] = $evalues["MemberName"];
	$data["MemberRelationshipId"] = $evalues["MemberRelationshipId"];
...
if($inlineedit != EDIT_INLINE)
{
	...
	if(!$pageObject->isAppearOnTabs("MemberName"))
		$xt->assign("MemberName_fieldblock",true);
	else
		$xt->assign("MemberName_tabfieldblock",true);
	$xt->assign("MemberName_label",true);
	if(isEnableSection508())
		$xt->assign_section("MemberName_label","<label for=\"".GetInputElementId("MemberName", $id, PAGE_EDIT)."\">","</label>");
		
	if(!$pageObject->isAppearOnTabs("MemberRelationshipId"))
		$xt->assign("MemberRelationshipId_fieldblock",true);
	else
		$xt->assign("MemberRelationshipId_tabfieldblock",true);
	$xt->assign("MemberRelationshipId_label",true);
	if(isEnableSection508())
		$xt->assign_section("MemberRelationshipId_label","<label for=\"".GetInputElementId("MemberRelationshipId", $id, PAGE_EDIT)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("Remark_label","<label for=\"".GetInputElementId("Remark", $id, PAGE_EDIT)."\">","</label>");
		

	$xt->assign("show_key1", htmlspecialchars($pageObject->showDBValue("FamilyMemberId", $data)));
...
//	detail tables
	...
	$keylink = "";
	$keylink.= "&key1=".htmlspecialchars(rawurlencode(@$data["FamilyMemberId"]));


//	FamilyMemberId - 
	$value = $pageObject->showDBValue("FamilyMemberId", $data, $keylink);
	$showValues["FamilyMemberId"] = $value;
	$showFields[] = "FamilyMemberId";
		$showRawValues["FamilyMemberId"] = substr($data["FamilyMemberId"],0,100);

//	DevoteeIdFamilyOwner - 
	$value = $pageObject->showDBValue("DevoteeIdFamilyOwner", $data, $keylink);
	$showValues["DevoteeIdFamilyOwner"] = $value;
	$showFields[] = "DevoteeIdFamilyOwner";
		$showRawValues["DevoteeIdFamilyOwner"] = substr($data["DevoteeIdFamilyOwner"],0,100);

//	MemberName - 
	$value = $pageObject->showDBValue("MemberName", $data, $keylink);
	$showValues["MemberName"] = $value;
	$showFields[] = "MemberName";
		$showRawValues["MemberName"] = substr($data["MemberName"],0,100);

//	MemberRelationshipId - 
	$value = $pageObject->showDBValue("MemberRelationshipId", $data, $keylink);
	$showValues["MemberRelationshipId"] = $value;
	$showFields[] = "MemberRelationshipId";
		$showRawValues["MemberRelationshipId"] = substr($data["MemberRelationshipId"],0,100);
...
//	DeovteeIdFamilyMember - 
	$value = $pageObject->showDBValue("DeovteeIdFamilyMember", $data, $keylink);
	$showValues["DeovteeIdFamilyMember"] = $value;
	$showFields[] = "DeovteeIdFamilyMember";
		$showRawValues["DeovteeIdFamilyMember"] = substr($data["DeovteeIdFamilyMember"],0,100);
...
if($pageObject->isShowDetailTables && $inlineedit!=EDIT_INLINE && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "devotee_family_member";
		$options['firstTime'] = 1;
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='devotee_family_member_view.php?".$viewlink."'\"");
