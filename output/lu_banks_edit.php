...
include("include/lu_banks_variables.php");
...
$layout->blocks["top"][] = "details";$page_layouts["lu_banks_edit"] = $layout;
...
$templatefile = ($inlineedit == EDIT_INLINE) ? "lu_banks_inline_edit.htm" : "lu_banks_edit.htm";
...
$skeys = "";
$savedKeys = array();
$keys["BankId"] = urldecode(postvalue("editid1"));
$savedKeys["BankId"] = urldecode(postvalue("editid1"));
...
//	processing BankId - begin
	$condition = 1;

	if($condition)
	{
		$control_BankId = $pageObject->getControl("BankId", $id);
		$control_BankId->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		//	update key value
		if($control_BankId->getWebValue()!==false)
			$keys["BankId"] = $control_BankId->getWebValue();
	}
//	processing BankId - end
	...
	//	do event
		...
		{		
			...
			//	processing BankId - begin
				...
				if($condition)
				{
					$control_BankId->afterSuccessfulSave();
				}
	//	processing BankId - end
			//	processing Name - begin
							$condition = 1;
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// key get query
	$keyGetQ = "";
		$keyGetQ.="editid1=".rawurldecode($keys["BankId"])."&";
	// cut last &
	$keyGetQ = substr($keyGetQ, 0, strlen($keyGetQ)-1);	
	// redirect
	header("Location: lu_banks_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: lu_banks_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
if($inlineedit != EDIT_INLINE)
{
	...
	if(!$pageObject->isAppearOnTabs("BankId"))
		$xt->assign("BankId_fieldblock",true);
	else
		$xt->assign("BankId_tabfieldblock",true);
	$xt->assign("BankId_label",true);
	if(isEnableSection508())
		$xt->assign_section("BankId_label","<label for=\"".GetInputElementId("BankId", $id, PAGE_EDIT)."\">","</label>");
	...
	$xt->assign("show_key1", htmlspecialchars($pageObject->showDBValue("BankId", $data)));
...
//	detail tables
	...
	$keylink = "";
	$keylink.= "&key1=".htmlspecialchars(rawurlencode(@$data["BankId"]));


//	BankId - 
	$value = $pageObject->showDBValue("BankId", $data, $keylink);
	$showValues["BankId"] = $value;
	$showFields[] = "BankId";
		$showRawValues["BankId"] = substr($data["BankId"],0,100);
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='lu_banks_view.php?".$viewlink."'\"");
