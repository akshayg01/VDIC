...
include("include/lu_marital_status_variables.php");
...
$layout->blocks["top"][] = "details";$page_layouts["lu_marital_status_edit"] = $layout;
...
$templatefile = ($inlineedit == EDIT_INLINE) ? "lu_marital_status_inline_edit.htm" : "lu_marital_status_edit.htm";
...
$skeys = "";
$savedKeys = array();
$keys["MaritalStatusId"] = urldecode(postvalue("editid1"));
$savedKeys["MaritalStatusId"] = urldecode(postvalue("editid1"));
...
//	processing MaritalStatusId - begin
	...
	if($condition)
	{
		...
		//	update key value
		if($control_MaritalStatusId->getWebValue()!==false)
			$keys["MaritalStatusId"] = $control_MaritalStatusId->getWebValue();
	}
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// key get query
	$keyGetQ = "";
		$keyGetQ.="editid1=".rawurldecode($keys["MaritalStatusId"])."&";
	// cut last &
	$keyGetQ = substr($keyGetQ, 0, strlen($keyGetQ)-1);	
	// redirect
	header("Location: lu_marital_status_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: lu_marital_status_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
if($inlineedit != EDIT_INLINE)
{
	...
	$xt->assign("show_key1", htmlspecialchars($pageObject->showDBValue("MaritalStatusId", $data)));
...
//	detail tables
	...
	$keylink = "";
	$keylink.= "&key1=".htmlspecialchars(rawurlencode(@$data["MaritalStatusId"]));
...
if($pageObject->isShowDetailTables && $inlineedit!=EDIT_INLINE && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "lu_marital_status";
		$options['firstTime'] = 1;
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='lu_marital_status_view.php?".$viewlink."'\"");
