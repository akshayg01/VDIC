...
include("include/lu_donation_purpose_variables.php");
...
$layout->blocks["top"][] = "details";$page_layouts["lu_donation_purpose_edit"] = $layout;
...
$templatefile = ($inlineedit == EDIT_INLINE) ? "lu_donation_purpose_inline_edit.htm" : "lu_donation_purpose_edit.htm";
...
$skeys = "";
$savedKeys = array();
$keys["DonationPurposeId"] = urldecode(postvalue("editid1"));
$savedKeys["DonationPurposeId"] = urldecode(postvalue("editid1"));
...
//	processing DonationPurposeId - begin
	...
	if($condition)
	{
		...
		//	update key value
		if($control_DonationPurposeId->getWebValue()!==false)
			$keys["DonationPurposeId"] = $control_DonationPurposeId->getWebValue();
	}
//	processing DonationPurposeId - end
//	processing PurposeTitle - begin
	$condition = 1;

	if($condition)
	{
		$control_PurposeTitle = $pageObject->getControl("PurposeTitle", $id);
		$control_PurposeTitle->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing PurposeTitle - end
	...
	//	processing DonationPurposeId - end
			//	processing PurposeTitle - begin
				...
				if($condition)
				{
					$control_PurposeTitle->afterSuccessfulSave();
				}
	//	processing PurposeTitle - end
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// key get query
	$keyGetQ = "";
		$keyGetQ.="editid1=".rawurldecode($keys["DonationPurposeId"])."&";
	// cut last &
	$keyGetQ = substr($keyGetQ, 0, strlen($keyGetQ)-1);	
	// redirect
	header("Location: lu_donation_purpose_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: lu_donation_purpose_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
if($readevalues)
{
	...
	$data["PurposeTitle"] = $evalues["PurposeTitle"];
}
...
if($inlineedit != EDIT_INLINE)
{
	...
	if(!$pageObject->isAppearOnTabs("PurposeTitle"))
		$xt->assign("PurposeTitle_fieldblock",true);
	else
		$xt->assign("PurposeTitle_tabfieldblock",true);
	$xt->assign("PurposeTitle_label",true);
	if(isEnableSection508())
		$xt->assign_section("PurposeTitle_label","<label for=\"".GetInputElementId("PurposeTitle", $id, PAGE_EDIT)."\">","</label>");
		

	$xt->assign("show_key1", htmlspecialchars($pageObject->showDBValue("DonationPurposeId", $data)));
...
//	detail tables
	...
	$keylink = "";
	$keylink.= "&key1=".htmlspecialchars(rawurlencode(@$data["DonationPurposeId"]));
...
//	PurposeTitle - 
	$value = $pageObject->showDBValue("PurposeTitle", $data, $keylink);
	$showValues["PurposeTitle"] = $value;
	$showFields[] = "PurposeTitle";
		$showRawValues["PurposeTitle"] = substr($data["PurposeTitle"],0,100);
/////////////////////////////////////////////////////////////
//	start inline output
/////////////////////////////////////////////////////////////
...
if($pageObject->isShowDetailTables && $inlineedit!=EDIT_INLINE && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "lu_donation_purpose";
		$options['firstTime'] = 1;
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='lu_donation_purpose_view.php?".$viewlink."'\"");
