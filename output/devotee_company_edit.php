...
include("include/devotee_company_variables.php");
...
$layout->blocks["top"][] = "details";$page_layouts["devotee_company_edit"] = $layout;
...
$templatefile = ($inlineedit == EDIT_INLINE) ? "devotee_company_inline_edit.htm" : "devotee_company_edit.htm";
...
$skeys = "";
$savedKeys = array();
$keys["DevoteeCompanyId"] = urldecode(postvalue("editid1"));
$savedKeys["DevoteeCompanyId"] = urldecode(postvalue("editid1"));
...
//	processing CompanyId - begin
	$condition = 1;

	if($condition)
	{
		$control_CompanyId = $pageObject->getControl("CompanyId", $id);
		$control_CompanyId->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing CompanyId - end
//	processing FromDate - begin
	$condition = 1;

	if($condition)
	{
		$control_FromDate = $pageObject->getControl("FromDate", $id);
		$control_FromDate->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing FromDate - end
//	processing ToDate - begin
	$condition = 1;

	if($condition)
	{
		$control_ToDate = $pageObject->getControl("ToDate", $id);
		$control_ToDate->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing ToDate - end
//	processing Designation - begin
	$condition = 1;

	if($condition)
	{
		$control_Designation = $pageObject->getControl("Designation", $id);
		$control_Designation->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing Designation - end
	...
	//	do event
		...
		{		
			...
			//	processing CompanyId - begin
				...
				if($condition)
				{
					$control_CompanyId->afterSuccessfulSave();
				}
	//	processing CompanyId - end
			//	processing FromDate - begin
							$condition = 1;
				...
				if($condition)
				{
					$control_FromDate->afterSuccessfulSave();
				}
	//	processing FromDate - end
			//	processing ToDate - begin
							$condition = 1;
				...
				if($condition)
				{
					$control_ToDate->afterSuccessfulSave();
				}
	//	processing ToDate - end
			//	processing Designation - begin
							$condition = 1;
				...
				if($condition)
				{
					$control_Designation->afterSuccessfulSave();
				}
	//	processing Designation - end
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// key get query
	$keyGetQ = "";
		$keyGetQ.="editid1=".rawurldecode($keys["DevoteeCompanyId"])."&";
	// cut last &
	$keyGetQ = substr($keyGetQ, 0, strlen($keyGetQ)-1);	
	// redirect
	header("Location: devotee_company_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: devotee_company_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
if($readevalues)
{
	$data["CompanyId"] = $evalues["CompanyId"];
	$data["FromDate"] = $evalues["FromDate"];
	$data["ToDate"] = $evalues["ToDate"];
...
if($inlineedit != EDIT_INLINE)
{
	...
	if(!$pageObject->isAppearOnTabs("CompanyId"))
		$xt->assign("CompanyId_fieldblock",true);
	else
		$xt->assign("CompanyId_tabfieldblock",true);
	$xt->assign("CompanyId_label",true);
	if(isEnableSection508())
		$xt->assign_section("CompanyId_label","<label for=\"".GetInputElementId("CompanyId", $id, PAGE_EDIT)."\">","</label>");
		
	if(!$pageObject->isAppearOnTabs("FromDate"))
		$xt->assign("FromDate_fieldblock",true);
	else
		$xt->assign("FromDate_tabfieldblock",true);
	$xt->assign("FromDate_label",true);
	if(isEnableSection508())
		$xt->assign_section("FromDate_label","<label for=\"".GetInputElementId("FromDate", $id, PAGE_EDIT)."\">","</label>");
		
	if(!$pageObject->isAppearOnTabs("ToDate"))
		$xt->assign("ToDate_fieldblock",true);
	else
		$xt->assign("ToDate_tabfieldblock",true);
	$xt->assign("ToDate_label",true);
	if(isEnableSection508())
		$xt->assign_section("ToDate_label","<label for=\"".GetInputElementId("ToDate", $id, PAGE_EDIT)."\">","</label>");
		
	if(!$pageObject->isAppearOnTabs("Designation"))
		$xt->assign("Designation_fieldblock",true);
	else
		$xt->assign("Designation_tabfieldblock",true);
	$xt->assign("Designation_label",true);
	if(isEnableSection508())
		$xt->assign_section("Designation_label","<label for=\"".GetInputElementId("Designation", $id, PAGE_EDIT)."\">","</label>");
		

	$xt->assign("show_key1", htmlspecialchars($pageObject->showDBValue("DevoteeCompanyId", $data)));
...
//	detail tables
	...
	$keylink = "";
	$keylink.= "&key1=".htmlspecialchars(rawurlencode(@$data["DevoteeCompanyId"]));


//	DevoteeCompanyId - 
	$value = $pageObject->showDBValue("DevoteeCompanyId", $data, $keylink);
	$showValues["DevoteeCompanyId"] = $value;
	$showFields[] = "DevoteeCompanyId";
		$showRawValues["DevoteeCompanyId"] = substr($data["DevoteeCompanyId"],0,100);
...
//	CompanyId - 
	$value = $pageObject->showDBValue("CompanyId", $data, $keylink);
	$showValues["CompanyId"] = $value;
	$showFields[] = "CompanyId";
		$showRawValues["CompanyId"] = substr($data["CompanyId"],0,100);

//	FromDate - Short Date
	$value = $pageObject->showDBValue("FromDate", $data, $keylink);
	$showValues["FromDate"] = $value;
	$showFields[] = "FromDate";
		$showRawValues["FromDate"] = substr($data["FromDate"],0,100);

//	ToDate - Short Date
	$value = $pageObject->showDBValue("ToDate", $data, $keylink);
	$showValues["ToDate"] = $value;
	$showFields[] = "ToDate";
		$showRawValues["ToDate"] = substr($data["ToDate"],0,100);

//	Designation - 
	$value = $pageObject->showDBValue("Designation", $data, $keylink);
	$showValues["Designation"] = $value;
	$showFields[] = "Designation";
		$showRawValues["Designation"] = substr($data["Designation"],0,100);
/////////////////////////////////////////////////////////////
//	start inline output
/////////////////////////////////////////////////////////////
...
if($pageObject->isShowDetailTables && $inlineedit!=EDIT_INLINE && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "devotee_company";
		$options['firstTime'] = 1;
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='devotee_company_view.php?".$viewlink."'\"");
