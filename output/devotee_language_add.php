...
add_nocache_headers();
include("include/devotee_language_variables.php");
...
$layout->blocks["top"][] = "details";$page_layouts["devotee_language_add"] = $layout;
...
if($inlineadd == ADD_INLINE)
	$templatefile = "devotee_language_inline_add.htm";
else
	$templatefile = "devotee_language_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: devotee_language_add.php');
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'devotee_language_edit.php?'.$keylink.'\'>'."Edit".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'devotee_language_view.php?'.$keylink.'\'>'."View".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: devotee_language_".$pageObject->getPageType().".php");
...
if(array_key_exists("copyid1",$_REQUEST) || array_key_exists("editid1",$_REQUEST))
{
	...
	{
		$copykeys["DevoteeLanguageId"]=postvalue("copyid1");
	}
	else
	{
		$copykeys["DevoteeLanguageId"]=postvalue("editid1");
	}
...
if($readavalues)
{
	$defvalues["LanguageId"]=@$avalues["LanguageId"];
...
if($inlineadd!=ADD_INLINE)
{
	...
	if(!$pageObject->isAppearOnTabs("LanguageId"))
		$xt->assign("LanguageId_fieldblock",true);
	else
		$xt->assign("LanguageId_tabfieldblock",true);
	$xt->assign("LanguageId_label",true);
	if(isEnableSection508())
		$xt->assign_section("LanguageId_label","<label for=\"".GetInputElementId("LanguageId", $id, PAGE_ADD)."\">","</label>");
...
if(@$_POST["a"]=="added" && ($inlineadd == ADD_INLINE || $inlineadd == ADD_MASTER || $inlineadd==ADD_POPUP)) 
{
	...
	$keylink="";
	$keylink.="&key1=".htmlspecialchars(rawurlencode(@$data["DevoteeLanguageId"]));
...
if($pageObject->isShowDetailTables && ($inlineadd==ADD_SIMPLE || $inlineadd==ADD_POPUP) && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "devotee_language";
		$options['firstTime'] = 1;
