...
$layout->blocks["top"][] = "details";$page_layouts["lu_banks_add"] = $layout;
...
if($inlineadd == ADD_INLINE)
	$templatefile = "lu_banks_inline_add.htm";
else
	$templatefile = "lu_banks_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: lu_banks_add.php');
...
if(@$_POST["a"]=="added")
{
	...
	$avalues=array();
	$blobfields=array();
//	processing BankId - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_BankId = $pageObject->getControl("BankId", $id);
		$control_BankId->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'lu_banks_edit.php?'.$keylink.'\'>'."Edit".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'lu_banks_view.php?'.$keylink.'\'>'."View".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: lu_banks_".$pageObject->getPageType().".php");
...
if(array_key_exists("copyid1",$_REQUEST) || array_key_exists("editid1",$_REQUEST))
{
	...
	{
		$copykeys["BankId"]=postvalue("copyid1");
	}
	else
	{
		$copykeys["BankId"]=postvalue("editid1");
	}
...
if($readavalues)
{
	$defvalues["BankId"]=@$avalues["BankId"];
...
if($inlineadd!=ADD_INLINE)
{
	...
	if(isEnableSection508())
		$xt->assign_section("BankId_label","<label for=\"".GetInputElementId("BankId", $id, PAGE_ADD)."\">","</label>");
