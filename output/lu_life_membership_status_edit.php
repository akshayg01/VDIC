...
include("include/lu_life_membership_status_variables.php");
...
$layout->blocks["top"][] = "details";$page_layouts["lu_life_membership_status_edit"] = $layout;
...
$templatefile = ($inlineedit == EDIT_INLINE) ? "lu_life_membership_status_inline_edit.htm" : "lu_life_membership_status_edit.htm";
...
$skeys = "";
$savedKeys = array();
$keys["LifeMembershipStatusId"] = urldecode(postvalue("editid1"));
$savedKeys["LifeMembershipStatusId"] = urldecode(postvalue("editid1"));
...
//	processing LifeMembershipStatusId - begin
	$condition = 1;

	if($condition)
	{
		$control_LifeMembershipStatusId = $pageObject->getControl("LifeMembershipStatusId", $id);
		$control_LifeMembershipStatusId->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		//	update key value
		if($control_LifeMembershipStatusId->getWebValue()!==false)
			$keys["LifeMembershipStatusId"] = $control_LifeMembershipStatusId->getWebValue();
	}
//	processing LifeMembershipStatusId - end
	...
	//	do event
		...
		{		
			...
			//	processing LifeMembershipStatusId - begin
				...
				if($condition)
				{
					$control_LifeMembershipStatusId->afterSuccessfulSave();
				}
	//	processing LifeMembershipStatusId - end
			//	processing status - begin
							$condition = 1;
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// key get query
	$keyGetQ = "";
		$keyGetQ.="editid1=".rawurldecode($keys["LifeMembershipStatusId"])."&";
	// cut last &
	$keyGetQ = substr($keyGetQ, 0, strlen($keyGetQ)-1);	
	// redirect
	header("Location: lu_life_membership_status_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: lu_life_membership_status_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
if($readevalues)
{
	$data["LifeMembershipStatusId"] = $evalues["LifeMembershipStatusId"];
...
if($inlineedit != EDIT_INLINE)
{
	...
	if(!$pageObject->isAppearOnTabs("LifeMembershipStatusId"))
		$xt->assign("LifeMembershipStatusId_fieldblock",true);
	else
		$xt->assign("LifeMembershipStatusId_tabfieldblock",true);
	$xt->assign("LifeMembershipStatusId_label",true);
	if(isEnableSection508())
		$xt->assign_section("LifeMembershipStatusId_label","<label for=\"".GetInputElementId("LifeMembershipStatusId", $id, PAGE_EDIT)."\">","</label>");
	...
	$xt->assign("show_key1", htmlspecialchars($pageObject->showDBValue("LifeMembershipStatusId", $data)));
...
//	detail tables
	...
	$keylink = "";
	$keylink.= "&key1=".htmlspecialchars(rawurlencode(@$data["LifeMembershipStatusId"]));


//	LifeMembershipStatusId - 
	$value = $pageObject->showDBValue("LifeMembershipStatusId", $data, $keylink);
	$showValues["LifeMembershipStatusId"] = $value;
	$showFields[] = "LifeMembershipStatusId";
		$showRawValues["LifeMembershipStatusId"] = substr($data["LifeMembershipStatusId"],0,100);
...
if($pageObject->isShowDetailTables && $inlineedit!=EDIT_INLINE && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "lu_life_membership_status";
		$options['firstTime'] = 1;
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='lu_life_membership_status_view.php?".$viewlink."'\"");
