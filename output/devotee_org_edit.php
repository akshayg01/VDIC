...
include("include/devotee_org_variables.php");
...
$layout->blocks["top"][] = "details";$page_layouts["devotee_org_edit"] = $layout;
...
$templatefile = ($inlineedit == EDIT_INLINE) ? "devotee_org_inline_edit.htm" : "devotee_org_edit.htm";
...
$skeys = "";
$savedKeys = array();
$keys["DevoteeOrgId"] = urldecode(postvalue("editid1"));
$savedKeys["DevoteeOrgId"] = urldecode(postvalue("editid1"));
...
//	processing OrgId - begin
	$condition = 1;

	if($condition)
	{
		$control_OrgId = $pageObject->getControl("OrgId", $id);
		$control_OrgId->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing OrgId - end
...
//	processing Role - begin
	$condition = 1;

	if($condition)
	{
		$control_Role = $pageObject->getControl("Role", $id);
		$control_Role->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);
...
//	processing Role - end
	...
	//	do event
		...
		{		
			...
			//	processing OrgId - begin
				...
				if($condition)
				{
					$control_OrgId->afterSuccessfulSave();
				}
	//	processing OrgId - end
			//	processing FromDate - begin
							$condition = 1;
	...
	//	processing ToDate - end
			//	processing Role - begin
				...
				if($condition)
				{
					$control_Role->afterSuccessfulSave();
				}
	//	processing Role - end
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// key get query
	$keyGetQ = "";
		$keyGetQ.="editid1=".rawurldecode($keys["DevoteeOrgId"])."&";
	// cut last &
	$keyGetQ = substr($keyGetQ, 0, strlen($keyGetQ)-1);	
	// redirect
	header("Location: devotee_org_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: devotee_org_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
if($inlineedit != EDIT_INLINE)
{
	...
	if(!$pageObject->isAppearOnTabs("OrgId"))
		$xt->assign("OrgId_fieldblock",true);
	else
		$xt->assign("OrgId_tabfieldblock",true);
	$xt->assign("OrgId_label",true);
	if(isEnableSection508())
		$xt->assign_section("OrgId_label","<label for=\"".GetInputElementId("OrgId", $id, PAGE_EDIT)."\">","</label>");
	...
	if(!$pageObject->isAppearOnTabs("Role"))
		$xt->assign("Role_fieldblock",true);
	else
		$xt->assign("Role_tabfieldblock",true);
	$xt->assign("Role_label",true);
	if(isEnableSection508())
		$xt->assign_section("Role_label","<label for=\"".GetInputElementId("Role", $id, PAGE_EDIT)."\">","</label>");
		

	$xt->assign("show_key1", htmlspecialchars($pageObject->showDBValue("DevoteeOrgId", $data)));
...
//	detail tables
	...
	$keylink = "";
	$keylink.= "&key1=".htmlspecialchars(rawurlencode(@$data["DevoteeOrgId"]));


//	DevoteeOrgId - 
	$value = $pageObject->showDBValue("DevoteeOrgId", $data, $keylink);
	$showValues["DevoteeOrgId"] = $value;
	$showFields[] = "DevoteeOrgId";
		$showRawValues["DevoteeOrgId"] = substr($data["DevoteeOrgId"],0,100);

//	OrgId - 
	$value = $pageObject->showDBValue("OrgId", $data, $keylink);
	$showValues["OrgId"] = $value;
	$showFields[] = "OrgId";
		$showRawValues["OrgId"] = substr($data["OrgId"],0,100);
...
//	Role - 
	$value = $pageObject->showDBValue("Role", $data, $keylink);
	$showValues["Role"] = $value;
	$showFields[] = "Role";
...
if($pageObject->isShowDetailTables && $inlineedit!=EDIT_INLINE && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "devotee_org";
		$options['firstTime'] = 1;
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='devotee_org_view.php?".$viewlink."'\"");
