...
$layout->blocks["top"][] = "details";$page_layouts["lu_request_types_edit"] = $layout;
...
$templatefile = ($inlineedit == EDIT_INLINE) ? "lu_request_types_inline_edit.htm" : "lu_request_types_edit.htm";
...
$skeys = "";
$savedKeys = array();
$keys["RequestTypeId"] = urldecode(postvalue("editid1"));
$savedKeys["RequestTypeId"] = urldecode(postvalue("editid1"));
...
//	processing RequestTypeId - begin
	$condition = 1;

	if($condition)
	{
		$control_RequestTypeId = $pageObject->getControl("RequestTypeId", $id);
		$control_RequestTypeId->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		//	update key value
		if($control_RequestTypeId->getWebValue()!==false)
			$keys["RequestTypeId"] = $control_RequestTypeId->getWebValue();
	}
//	processing RequestTypeId - end
//	processing RequestType - begin
	$condition = 1;

	if($condition)
	{
		$control_RequestType = $pageObject->getControl("RequestType", $id);
		$control_RequestType->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing RequestType - end
	...
	//	do event
		...
		{		
			...
			//	processing RequestTypeId - begin
				...
				if($condition)
				{
					$control_RequestTypeId->afterSuccessfulSave();
				}
	//	processing RequestTypeId - end
			//	processing RequestType - begin
							$condition = 1;
				...
				if($condition)
				{
					$control_RequestType->afterSuccessfulSave();
				}
	//	processing RequestType - end
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// key get query
	$keyGetQ = "";
		$keyGetQ.="editid1=".rawurldecode($keys["RequestTypeId"])."&";
	// cut last &
	$keyGetQ = substr($keyGetQ, 0, strlen($keyGetQ)-1);	
	// redirect
	header("Location: lu_request_types_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: lu_request_types_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
if($readevalues)
{
	$data["RequestTypeId"] = $evalues["RequestTypeId"];
	$data["RequestType"] = $evalues["RequestType"];
}
...
if($inlineedit != EDIT_INLINE)
{
	...
	if(!$pageObject->isAppearOnTabs("RequestTypeId"))
		$xt->assign("RequestTypeId_fieldblock",true);
	else
		$xt->assign("RequestTypeId_tabfieldblock",true);
	$xt->assign("RequestTypeId_label",true);
	if(isEnableSection508())
		$xt->assign_section("RequestTypeId_label","<label for=\"".GetInputElementId("RequestTypeId", $id, PAGE_EDIT)."\">","</label>");
		
	if(!$pageObject->isAppearOnTabs("RequestType"))
		$xt->assign("RequestType_fieldblock",true);
	else
		$xt->assign("RequestType_tabfieldblock",true);
	$xt->assign("RequestType_label",true);
	if(isEnableSection508())
		$xt->assign_section("RequestType_label","<label for=\"".GetInputElementId("RequestType", $id, PAGE_EDIT)."\">","</label>");
		

	$xt->assign("show_key1", htmlspecialchars($pageObject->showDBValue("RequestTypeId", $data)));
...
//	detail tables
	...
	$keylink = "";
	$keylink.= "&key1=".htmlspecialchars(rawurlencode(@$data["RequestTypeId"]));


//	RequestTypeId - 
	$value = $pageObject->showDBValue("RequestTypeId", $data, $keylink);
	$showValues["RequestTypeId"] = $value;
	$showFields[] = "RequestTypeId";
		$showRawValues["RequestTypeId"] = substr($data["RequestTypeId"],0,100);

//	RequestType - 
	$value = $pageObject->showDBValue("RequestType", $data, $keylink);
	$showValues["RequestType"] = $value;
	$showFields[] = "RequestType";
		$showRawValues["RequestType"] = substr($data["RequestType"],0,100);
/////////////////////////////////////////////////////////////
//	start inline output
/////////////////////////////////////////////////////////////
...
if($pageObject->isShowDetailTables && $inlineedit!=EDIT_INLINE && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "lu_request_types";
		$options['firstTime'] = 1;
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='lu_request_types_view.php?".$viewlink."'\"");
