...
$layout->blocks["top"][] = "details";$page_layouts["lu_life_membership_status_add"] = $layout;
...
if($inlineadd == ADD_INLINE)
	$templatefile = "lu_life_membership_status_inline_add.htm";
else
	$templatefile = "lu_life_membership_status_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: lu_life_membership_status_add.php');
...
if(@$_POST["a"]=="added")
{
	...
	$avalues=array();
	$blobfields=array();
//	processing LifeMembershipStatusId - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_LifeMembershipStatusId = $pageObject->getControl("LifeMembershipStatusId", $id);
		$control_LifeMembershipStatusId->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'lu_life_membership_status_edit.php?'.$keylink.'\'>'."Edit".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'lu_life_membership_status_view.php?'.$keylink.'\'>'."View".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: lu_life_membership_status_".$pageObject->getPageType().".php");
...
if(array_key_exists("copyid1",$_REQUEST) || array_key_exists("editid1",$_REQUEST))
{
	...
	{
		$copykeys["LifeMembershipStatusId"]=postvalue("copyid1");
	}
	else
	{
		$copykeys["LifeMembershipStatusId"]=postvalue("editid1");
	}
	...
	if(!$defvalues)
		$defvalues=array();
//	clear key fields
	$defvalues["LifeMembershipStatusId"]="";
...
if($readavalues)
{
	$defvalues["LifeMembershipStatusId"]=@$avalues["LifeMembershipStatusId"];
...
if($inlineadd!=ADD_INLINE)
{
	...
	if(isEnableSection508())
		$xt->assign_section("LifeMembershipStatusId_label","<label for=\"".GetInputElementId("LifeMembershipStatusId", $id, PAGE_ADD)."\">","</label>");
