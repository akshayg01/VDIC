...
$layout->blocks["top"][] = "details";$page_layouts["vw_counsellee_temp_add"] = $layout;
...
$layout->blocks["bare"][] = "1";$page_layouts["vw_counsellee_temp_add_New_tab1"] = $layout;
...
if($inlineadd == ADD_INLINE)
	$templatefile = "vw_counsellee_temp_inline_add.htm";
else
	$templatefile = "vw_counsellee_temp_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: vw_counsellee_temp_add.php');
...
//	processing IsCounsellor - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd!=ADD_INLINE;
	if($inlineAddOption)
	{
		$control_IsCounsellor = $pageObject->getControl("IsCounsellor", $id);
		$control_IsCounsellor->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing IsCounsellor - end
...
//	processing IsCounsellor - start
			...
			if($inlineAddOption)
			{
				$control_IsCounsellor->afterSuccessfulSave();
			}
//	processing IsCounsellor - end
//	processing TitleId - start
			$inlineAddOption = true;
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'vw_counsellee_temp_edit.php?'.$keylink.'\'>'."Edit".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'vw_counsellee_temp_view.php?'.$keylink.'\'>'."View".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: vw_counsellee_temp_".$pageObject->getPageType().".php");
...
{
	$defvalues["IsCounsellor"] = "Devotee.DevoteeID";
}
...
if($readavalues)
{
	...
	$defvalues["IsCounsellor"]=@$avalues["IsCounsellor"];
}
...
if($inlineadd!=ADD_INLINE)
{
	...
	if(!$pageObject->isAppearOnTabs("IsCounsellor"))
		$xt->assign("IsCounsellor_fieldblock",true);
	else
		$xt->assign("IsCounsellor_tabfieldblock",true);
	$xt->assign("IsCounsellor_label",true);
	if(isEnableSection508())
		$xt->assign_section("IsCounsellor_label","<label for=\"".GetInputElementId("IsCounsellor", $id, PAGE_ADD)."\">","</label>");
...
//	show readonly fields
	...
	$pageObject->readOnlyFields["CounsellorDevoteeID"] = htmlspecialchars($pageObject->showDBValue("CounsellorDevoteeID", $defvalues));
	$pageObject->readOnlyFields["IsCounsellor"] = htmlspecialchars($pageObject->showDBValue("IsCounsellor", $defvalues));
$linkdata="";
