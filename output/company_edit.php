...
$skeys = "";
$savedKeys = array();
$keys["CompanyId"] = urldecode(postvalue("editid1"));
$savedKeys["CompanyId"] = urldecode(postvalue("editid1"));
...
//	processing CompanyName - begin
	$condition = 1;

	if($condition)
	{
		$control_CompanyName = $pageObject->getControl("CompanyName", $id);
		$control_CompanyName->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing CompanyName - end
...
//	processing Address Line1 - begin
	$condition = 1;

	if($condition)
	{
		$control_Address_Line1 = $pageObject->getControl("Address Line1", $id);
		$control_Address_Line1->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing Address Line1 - end
//	processing Address Line2 - begin
	$condition = 1;

	if($condition)
	{
		$control_Address_Line2 = $pageObject->getControl("Address Line2", $id);
		$control_Address_Line2->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing Address Line2 - end
...
//	processing City - end
//	processing PIN Code - begin
	$condition = 1;

	if($condition)
	{
		$control_PIN_Code = $pageObject->getControl("PIN Code", $id);
		$control_PIN_Code->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing PIN Code - end
...
//	processing Country - end
	...
	//	do event
		...
		{		
			...
			//	processing CompanyName - begin
				...
				if($condition)
				{
					$control_CompanyName->afterSuccessfulSave();
				}
	//	processing CompanyName - end
			//	processing Remark - begin
							$condition = 1;
	...
	//	processing Remark - end
			//	processing Address Line1 - begin
				...
				if($condition)
				{
					$control_Address_Line1->afterSuccessfulSave();
				}
	//	processing Address Line1 - end
			//	processing Address Line2 - begin
							$condition = 1;
				...
				if($condition)
				{
					$control_Address_Line2->afterSuccessfulSave();
				}
	//	processing Address Line2 - end
			//	processing City - begin
							$condition = 1;
	...
	//	processing City - end
			//	processing PIN Code - begin
				...
				if($condition)
				{
					$control_PIN_Code->afterSuccessfulSave();
				}
	//	processing PIN Code - end
			//	processing State - begin
							$condition = 1;
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// key get query
	$keyGetQ = "";
		$keyGetQ.="editid1=".rawurldecode($keys["CompanyId"])."&";
	// cut last &
...
if($readevalues)
{
	$data["CompanyName"] = $evalues["CompanyName"];
	$data["Remark"] = $evalues["Remark"];
	$data["Address Line1"] = $evalues["Address Line1"];
	$data["Address Line2"] = $evalues["Address Line2"];
	$data["City"] = $evalues["City"];
	$data["PIN Code"] = $evalues["PIN Code"];
...
if($inlineedit != EDIT_INLINE)
{
	...
	if(!$pageObject->isAppearOnTabs("CompanyName"))
		$xt->assign("CompanyName_fieldblock",true);
	else
		$xt->assign("CompanyName_tabfieldblock",true);
	$xt->assign("CompanyName_label",true);
	if(isEnableSection508())
		$xt->assign_section("CompanyName_label","<label for=\"".GetInputElementId("CompanyName", $id, PAGE_EDIT)."\">","</label>");
	...
	if(!$pageObject->isAppearOnTabs("Address Line1"))
		$xt->assign("Address_Line1_fieldblock",true);
	else
		$xt->assign("Address_Line1_tabfieldblock",true);
	$xt->assign("Address_Line1_label",true);
	if(isEnableSection508())
		$xt->assign_section("Address_Line1_label","<label for=\"".GetInputElementId("Address Line1", $id, PAGE_EDIT)."\">","</label>");
		
	if(!$pageObject->isAppearOnTabs("Address Line2"))
		$xt->assign("Address_Line2_fieldblock",true);
	else
		$xt->assign("Address_Line2_tabfieldblock",true);
	$xt->assign("Address_Line2_label",true);
	if(isEnableSection508())
		$xt->assign_section("Address_Line2_label","<label for=\"".GetInputElementId("Address Line2", $id, PAGE_EDIT)."\">","</label>");
	...
	if(!$pageObject->isAppearOnTabs("PIN Code"))
		$xt->assign("PIN_Code_fieldblock",true);
	else
		$xt->assign("PIN_Code_tabfieldblock",true);
	$xt->assign("PIN_Code_label",true);
	if(isEnableSection508())
		$xt->assign_section("PIN_Code_label","<label for=\"".GetInputElementId("PIN Code", $id, PAGE_EDIT)."\">","</label>");
	...
	$xt->assign("show_key1", htmlspecialchars($pageObject->showDBValue("CompanyId", $data)));
...
//	detail tables
	...
	$keylink = "";
	$keylink.= "&key1=".htmlspecialchars(rawurlencode(@$data["CompanyId"]));
...
//	CompanyName - 
	$value = $pageObject->showDBValue("CompanyName", $data, $keylink);
	$showValues["CompanyName"] = $value;
	$showFields[] = "CompanyName";
		$showRawValues["CompanyName"] = substr($data["CompanyName"],0,100);
...
//	Address Line1 - 
	$value = $pageObject->showDBValue("Address Line1", $data, $keylink);
	$showValues["Address Line1"] = $value;
	$showFields[] = "Address Line1";
		$showRawValues["Address Line1"] = substr($data["Address Line1"],0,100);

//	Address Line2 - 
	$value = $pageObject->showDBValue("Address Line2", $data, $keylink);
	$showValues["Address Line2"] = $value;
	$showFields[] = "Address Line2";
		$showRawValues["Address Line2"] = substr($data["Address Line2"],0,100);
...
//	PIN Code - 
	$value = $pageObject->showDBValue("PIN Code", $data, $keylink);
	$showValues["PIN Code"] = $value;
	$showFields[] = "PIN Code";
		$showRawValues["PIN Code"] = substr($data["PIN Code"],0,100);
