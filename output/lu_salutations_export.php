...
$layout->blocks["top"][] = "export";$page_layouts["lu_salutations_export"] = $layout;
...
//	process selection
	...
	if (@$_REQUEST["mdelete"])
	{
		...
		{
			$keys=array();
			$keys["SalutationId"] = refine($_REQUEST["mdelete1"][mdeleteIndex($ind)]);
			$selected_recs[] = $keys;
		}
	}
	elseif(@$_REQUEST["selection"])
	{
		foreach(@$_REQUEST["selection"] as $keyblock)
		{
			...
			if(count($arr)<1)
				continue;
			$keys = array();
			$keys["SalutationId"] = urldecode($arr[0]);
			$selected_recs[] = $keys;
		}
	}
...
$xt->display("lu_salutations_export.htm");

function ExportToExcel_old($cipherer)
{
	global $cCharset;
	header("Content-Type: application/vnd.ms-excel");
	header("Content-Disposition: attachment;Filename=lu_salutations.xls");
...
function ExportToWord($cipherer)
{
	...
	header("Content-Disposition: attachment;Filename=lu_salutations.doc");
...
function ExportToXML($cipherer)
{
	...
	header("Content-Disposition: attachment;Filename=lu_salutations.xml");
	...
	while((!$nPageSize || $i<$nPageSize) && $row)
	{
		$values = array();
			...
			$values["SalutationId"] = $pageObject->showDBValue("SalutationId", $row);
			$values["salutation"] = $pageObject->showDBValue("salutation", $row);
...
function ExportToCSV($cipherer)
{
	...
	header("Content-Disposition: attachment;Filename=lu_salutations.csv");
...
// write data rows
	...
	while((!$nPageSize || $iNumberOfRows < $nPageSize) && $row)
	{
		$values = array();
			...
			$values["SalutationId"] = $pageObject->getViewControl("SalutationId")->showDBValue($row, "");
			$values["salutation"] = $pageObject->getViewControl("salutation")->showDBValue($row, "");
		...
		if ($eventRes)
		{
			...
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["SalutationId"]).'"';
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["salutation"]).'"';
			echo $outstr;
		}
...
// write header
	...
	if($_REQUEST["type"]=="excel")
	{
		...
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Salutation Id").'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Salutation").'</td>';	
	}
	else
	{
		echo "<td>"."Gender"."</td>";
		echo "<td>"."Salutation Id"."</td>";
		...
		if ($eventRes)
		{
			...
			echo '</td>';
					...
					echo '<td>';
									...
									if($_REQUEST["type"]=="excel")
						echo PrepareForExcel($values["salutation"]);
					else
						echo $values["salutation"];
			echo '</td>';
			echo "</tr>";
		}
