...
add_nocache_headers();
include("include/organization_variables.php");
...
$layout->blocks["top"][] = "details";$page_layouts["organization_add"] = $layout;
...
if($inlineadd == ADD_INLINE)
	$templatefile = "organization_inline_add.htm";
else
	$templatefile = "organization_add.htm";
...
//	processing OrgName - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_OrgName = $pageObject->getControl("OrgName", $id);
		$control_OrgName->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing OrgName - end
//	processing OrgOwnerId - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_OrgOwnerId = $pageObject->getControl("OrgOwnerId", $id);
		$control_OrgOwnerId->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing OrgOwnerId - end
...
//	processing OrgName - start
			...
			if($inlineAddOption)
			{
				$control_OrgName->afterSuccessfulSave();
			}
//	processing OrgName - end
//	processing OrgOwnerId - start
			$inlineAddOption = true;
			if($inlineAddOption)
			{
				$control_OrgOwnerId->afterSuccessfulSave();
			}
//	processing OrgOwnerId - end
//	processing Location - start
			$inlineAddOption = true;
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'organization_edit.php?'.$keylink.'\'>'."Edit".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'organization_view.php?'.$keylink.'\'>'."View".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: organization_".$pageObject->getPageType().".php");
...
if(array_key_exists("copyid1",$_REQUEST) || array_key_exists("editid1",$_REQUEST))
{
	...
	{
		$copykeys["OrgId"]=postvalue("copyid1");
	}
	else
	{
		$copykeys["OrgId"]=postvalue("editid1");
	}
...
if($readavalues)
{
	...
	$defvalues["OrgName"]=@$avalues["OrgName"];
	$defvalues["OrgOwnerId"]=@$avalues["OrgOwnerId"];
...
if($inlineadd!=ADD_INLINE)
{
	...
	if(!$pageObject->isAppearOnTabs("OrgName"))
		$xt->assign("OrgName_fieldblock",true);
	else
		$xt->assign("OrgName_tabfieldblock",true);
	$xt->assign("OrgName_label",true);
	if(isEnableSection508())
		$xt->assign_section("OrgName_label","<label for=\"".GetInputElementId("OrgName", $id, PAGE_ADD)."\">","</label>");
	
	if(!$pageObject->isAppearOnTabs("OrgOwnerId"))
		$xt->assign("OrgOwnerId_fieldblock",true);
	else
		$xt->assign("OrgOwnerId_tabfieldblock",true);
	$xt->assign("OrgOwnerId_label",true);
	if(isEnableSection508())
		$xt->assign_section("OrgOwnerId_label","<label for=\"".GetInputElementId("OrgOwnerId", $id, PAGE_ADD)."\">","</label>");
...
if(@$_POST["a"]=="added" && ($inlineadd == ADD_INLINE || $inlineadd == ADD_MASTER || $inlineadd==ADD_POPUP)) 
{
	...
	$showDetailKeys["devotee_org"]["masterkey1"] = $data["OrgId"];	

	$keylink="";
	$keylink.="&key1=".htmlspecialchars(rawurlencode(@$data["OrgId"]));
...
//	OrgName
	...
	if($display)
	{	
		$value = $pageObject->showDBValue("OrgName", $data, $keylink);
		$showValues["OrgName"] = $value;
		$showFields[] = "OrgName";
	}	
//	OrgOwnerId
	$display = false;
	...
	if($display)
	{	
		$value = $pageObject->showDBValue("OrgOwnerId", $data, $keylink);
		$showValues["OrgOwnerId"] = $value;
		$showFields[] = "OrgOwnerId";
	}	
//	Location
	$display = false;
	if($inlineadd==ADD_MASTER)
	...
	{	
		...
		$showRawValues["OrgName"] = substr($data["OrgName"],0,100);
		$showRawValues["OrgOwnerId"] = substr($data["OrgOwnerId"],0,100);
...
if($pageObject->isShowDetailTables && ($inlineadd==ADD_SIMPLE || $inlineadd==ADD_POPUP) && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "organization";
		$options['firstTime'] = 1;
