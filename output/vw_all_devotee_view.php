...
$layout->blocks["top"][] = "details";$page_layouts["vw_all_devotee_view"] = $layout;
...
if (!sizeof($data)) {
	header("Location: vw_all_devotee_list.php?a=return");
	exit();
}
...
$fieldsArr[] = $arr;
$arr = array();
$arr['fName'] = "Counsellor";
$arr['viewFormat'] = $pageObject->pSet->getViewFormat("Counsellor");
$fieldsArr[] = $arr;
$arr = array();
...
//Counsellor - 
	...
	$value = $pageObject->showDBValue("Counsellor", $data, $keylink);
	if($mainTableOwnerID=="Counsellor")
		$ownerIdValue=$value;
	$xt->assign("Counsellor_value",$value);
	if(!$pageObject->isAppearOnTabs("Counsellor"))
		$xt->assign("Counsellor_fieldblock",true);
	else
		$xt->assign("Counsellor_tabfieldblock",true);
////////////////////////////////////////////
...
if($pageObject->isShowDetailTables && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "vw_all_devotee";
		$options['firstTime'] = 1;
...
$xt->assign("editlink_attrs","id=\"editLink".$id."\" name=\"editLink".$id."\" onclick=\"window.location.href='vw_all_devotee_edit.php?".$editlink."'\"");
