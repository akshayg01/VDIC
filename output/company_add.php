...
if(@$_POST["a"]=="added")
{
	...
	$avalues=array();
	$blobfields=array();
//	processing CompanyName - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_CompanyName = $pageObject->getControl("CompanyName", $id);
		$control_CompanyName->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing CompanyName - end
//	processing Address Line1 - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_Address_Line1 = $pageObject->getControl("Address Line1", $id);
		$control_Address_Line1->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing Address Line1 - end
//	processing Address Line2 - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_Address_Line2 = $pageObject->getControl("Address Line2", $id);
		$control_Address_Line2->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
...
//	processing City - end
//	processing PIN Code - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_PIN_Code = $pageObject->getControl("PIN Code", $id);
		$control_PIN_Code->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
...
if(array_key_exists("copyid1",$_REQUEST) || array_key_exists("editid1",$_REQUEST))
{
	...
	{
		$copykeys["CompanyId"]=postvalue("copyid1");
	}
	else
	{
		$copykeys["CompanyId"]=postvalue("editid1");
	}
...
if($readavalues)
{
	$defvalues["CompanyName"]=@$avalues["CompanyName"];
	$defvalues["Remark"]=@$avalues["Remark"];
	$defvalues["Address Line1"]=@$avalues["Address Line1"];
	$defvalues["Address Line2"]=@$avalues["Address Line2"];
	$defvalues["City"]=@$avalues["City"];
	$defvalues["PIN Code"]=@$avalues["PIN Code"];
...
if($inlineadd!=ADD_INLINE)
{
	...
	if(isEnableSection508())
		$xt->assign_section("CompanyName_label","<label for=\"".GetInputElementId("CompanyName", $id, PAGE_ADD)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("Remark_label","<label for=\"".GetInputElementId("Remark", $id, PAGE_ADD)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("Address_Line1_label","<label for=\"".GetInputElementId("Address Line1", $id, PAGE_ADD)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("Address_Line2_label","<label for=\"".GetInputElementId("Address Line2", $id, PAGE_ADD)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("PIN_Code_label","<label for=\"".GetInputElementId("PIN Code", $id, PAGE_ADD)."\">","</label>");
...
function GetAddedDataLookupQuery($pageObject, $keys, $forLookup)
{
	...
	if(postvalue("table")=="company" && postvalue("field")=="CompanyId") 
	{
		$lookupFieldName = "CompanyId";
		$lookupTable = "company";
		$linkFieldName = "CompanyId";
		$dispfield = "CompanyName";
		...
		if($nLookupType == LT_QUERY)
		{
			if($lookupPSet->getCustomDisplay($lookupFieldName))
				$pageObject->pSet->getSQLQuery()->AddCustomExpression($dispfield, $pageObject->pSet, "company", "CompanyId");
			...
			$LookupSQL = "select ";
			$LookupSQL .= GetFullFieldName("CompanyId", $strTableName, true);
					if($linkFieldName != $dispfield)
				$LookupSQL .= ",CompanyName";
		}
	}
