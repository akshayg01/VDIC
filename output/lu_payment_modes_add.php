...
$layout->blocks["top"][] = "details";$page_layouts["lu_payment_modes_add"] = $layout;
...
if($inlineadd == ADD_INLINE)
	$templatefile = "lu_payment_modes_inline_add.htm";
else
	$templatefile = "lu_payment_modes_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: lu_payment_modes_add.php');
...
if(@$_POST["a"]=="added")
{
	...
	$avalues=array();
	$blobfields=array();
//	processing PaymentModeId - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd==ADD_INLINE;
	if($inlineAddOption)
	{
		$control_PaymentModeId = $pageObject->getControl("PaymentModeId", $id);
		$control_PaymentModeId->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'lu_payment_modes_edit.php?'.$keylink.'\'>'."Edit".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'lu_payment_modes_view.php?'.$keylink.'\'>'."View".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: lu_payment_modes_".$pageObject->getPageType().".php");
...
if(array_key_exists("copyid1",$_REQUEST) || array_key_exists("editid1",$_REQUEST))
{
	...
	{
		$copykeys["PaymentModeId"]=postvalue("copyid1");
	}
	else
	{
		$copykeys["PaymentModeId"]=postvalue("editid1");
	}
...
if($inlineadd!=ADD_INLINE)
{
	...
	if(isEnableSection508())
		$xt->assign_section("Mode_label","<label for=\"".GetInputElementId("Mode", $id, PAGE_ADD)."\">","</label>");
