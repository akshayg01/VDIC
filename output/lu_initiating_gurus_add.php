...
$layout->blocks["top"][] = "details";$page_layouts["lu_initiating_gurus_add"] = $layout;
...
if($inlineadd == ADD_INLINE)
	$templatefile = "lu_initiating_gurus_inline_add.htm";
else
	$templatefile = "lu_initiating_gurus_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: lu_initiating_gurus_add.php');
...
if(@$_POST["a"]=="added")
{
	...
	$avalues=array();
	$blobfields=array();
//	processing GurumaharajId - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_GurumaharajId = $pageObject->getControl("GurumaharajId", $id);
		$control_GurumaharajId->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'lu_initiating_gurus_edit.php?'.$keylink.'\'>'."Edit".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'lu_initiating_gurus_view.php?'.$keylink.'\'>'."View".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: lu_initiating_gurus_".$pageObject->getPageType().".php");
...
if(array_key_exists("copyid1",$_REQUEST) || array_key_exists("editid1",$_REQUEST))
{
	...
	{
		$copykeys["GurumaharajId"]=postvalue("copyid1");
	}
	else
	{
		$copykeys["GurumaharajId"]=postvalue("editid1");
	}
...
if($readavalues)
{
	$defvalues["GurumaharajId"]=@$avalues["GurumaharajId"];
...
if($inlineadd!=ADD_INLINE)
{
	...
	if(isEnableSection508())
		$xt->assign_section("GurumaharajId_label","<label for=\"".GetInputElementId("GurumaharajId", $id, PAGE_ADD)."\">","</label>");
