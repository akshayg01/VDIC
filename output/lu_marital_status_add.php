...
$layout->blocks["top"][] = "details";$page_layouts["lu_marital_status_add"] = $layout;
...
if($inlineadd == ADD_INLINE)
	$templatefile = "lu_marital_status_inline_add.htm";
else
	$templatefile = "lu_marital_status_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: lu_marital_status_add.php');
...
//	processing MaritalStatus - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_MaritalStatus = $pageObject->getControl("MaritalStatus", $id);
		$control_MaritalStatus->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'lu_marital_status_edit.php?'.$keylink.'\'>'."Edit".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'lu_marital_status_view.php?'.$keylink.'\'>'."View".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: lu_marital_status_".$pageObject->getPageType().".php");
...
if(array_key_exists("copyid1",$_REQUEST) || array_key_exists("editid1",$_REQUEST))
{
	...
	{
		$copykeys["MaritalStatusId"]=postvalue("copyid1");
	}
	else
	{
		$copykeys["MaritalStatusId"]=postvalue("editid1");
	}
...
if($readavalues)
{
	...
	$defvalues["MaritalStatus"]=@$avalues["MaritalStatus"];
}
...
if($inlineadd!=ADD_INLINE)
{
	...
	if(isEnableSection508())
		$xt->assign_section("MaritalStatus_label","<label for=\"".GetInputElementId("MaritalStatus", $id, PAGE_ADD)."\">","</label>");
