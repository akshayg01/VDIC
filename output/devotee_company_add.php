...
$layout->blocks["top"][] = "details";$page_layouts["devotee_company_add"] = $layout;
...
if($inlineadd == ADD_INLINE)
	$templatefile = "devotee_company_inline_add.htm";
else
	$templatefile = "devotee_company_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: devotee_company_add.php');
...
if(@$_POST["a"]=="added")
{
	...
	$avalues=array();
	$blobfields=array();
//	processing CompanyId - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_CompanyId = $pageObject->getControl("CompanyId", $id);
		$control_CompanyId->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing CompanyId - end
//	processing FromDate - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_FromDate = $pageObject->getControl("FromDate", $id);
		$control_FromDate->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing FromDate - end
//	processing ToDate - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_ToDate = $pageObject->getControl("ToDate", $id);
		$control_ToDate->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
...
//	processing Designation - start
	...
	if($inlineAddOption)
	{
		...
		$control_Designation->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'devotee_company_edit.php?'.$keylink.'\'>'."Edit".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'devotee_company_view.php?'.$keylink.'\'>'."View".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: devotee_company_".$pageObject->getPageType().".php");
...
if(array_key_exists("copyid1",$_REQUEST) || array_key_exists("editid1",$_REQUEST))
{
	...
	{
		$copykeys["DevoteeCompanyId"]=postvalue("copyid1");
	}
	else
	{
		$copykeys["DevoteeCompanyId"]=postvalue("editid1");
	}
...
if($readavalues)
{
	$defvalues["CompanyId"]=@$avalues["CompanyId"];
	$defvalues["FromDate"]=@$avalues["FromDate"];
	$defvalues["ToDate"]=@$avalues["ToDate"];
	$defvalues["Designation"]=@$avalues["Designation"];
}
...
if($inlineadd!=ADD_INLINE)
{
	...
	if(isEnableSection508())
		$xt->assign_section("CompanyId_label","<label for=\"".GetInputElementId("CompanyId", $id, PAGE_ADD)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("FromDate_label","<label for=\"".GetInputElementId("FromDate", $id, PAGE_ADD)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("ToDate_label","<label for=\"".GetInputElementId("ToDate", $id, PAGE_ADD)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("Designation_label","<label for=\"".GetInputElementId("Designation", $id, PAGE_ADD)."\">","</label>");
