...
$layout->blocks["top"][] = "details";$page_layouts["vw_OwnProfile_edit"] = $layout;
...
$templatefile = "vw_OwnProfile_edit.htm";
...
//	processing MaritalStatus - begin
	$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode

	if($condition)
	{
		$control_MaritalStatus = $pageObject->getControl("MaritalStatus", $id);
		$control_MaritalStatus->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing MaritalStatus - end
...
//	processing SpouseName - begin
	$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode

	if($condition)
	{
		$control_SpouseName = $pageObject->getControl("SpouseName", $id);
		$control_SpouseName->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing SpouseName - end
...
//	processing EmailSecondary - end
	...
	//	processing MaritalStatusId - end
			//	processing MaritalStatus - begin
				...
				if($condition)
				{
					$control_MaritalStatus->afterSuccessfulSave();
				}
	//	processing MaritalStatus - end
			//	processing DateofMarriage - begin
							$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode
	...
	//	processing SpouseDevoteeId - end
			//	processing SpouseName - begin
				...
				if($condition)
				{
					$control_SpouseName->afterSuccessfulSave();
				}
	//	processing SpouseName - end
			//	processing MobilePhone - begin
							$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// redirect
	header("Location: vw_OwnProfile_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: vw_OwnProfile_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
if($inlineedit != EDIT_INLINE)
{
	...
	if(!$pageObject->isAppearOnTabs("MaritalStatus"))
		$xt->assign("MaritalStatus_fieldblock",true);
	else
		$xt->assign("MaritalStatus_tabfieldblock",true);
	$xt->assign("MaritalStatus_label",true);
	if(isEnableSection508())
		$xt->assign_section("MaritalStatus_label","<label for=\"".GetInputElementId("MaritalStatus", $id, PAGE_EDIT)."\">","</label>");
	...
	if(!$pageObject->isAppearOnTabs("SpouseName"))
		$xt->assign("SpouseName_fieldblock",true);
	else
		$xt->assign("SpouseName_tabfieldblock",true);
	$xt->assign("SpouseName_label",true);
	if(isEnableSection508())
		$xt->assign_section("SpouseName_label","<label for=\"".GetInputElementId("SpouseName", $id, PAGE_EDIT)."\">","</label>");
...
//	MaritalStatus - 
	$value = $pageObject->showDBValue("MaritalStatus", $data, $keylink);
	$showValues["MaritalStatus"] = $value;
	$showFields[] = "MaritalStatus";
...
//	CounsellorName - 
	$value = $pageObject->showDBValue("CounsellorName", $data, $keylink);
	$showValues["CounsellorName"] = $value;
	$showFields[] = "CounsellorName";
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='vw_OwnProfile_view.php?".$viewlink."'\"");
