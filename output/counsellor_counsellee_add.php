...
$layout->blocks["top"][] = "details";$page_layouts["counsellor_counsellee_add"] = $layout;
...
if($inlineadd == ADD_INLINE)
	$templatefile = "counsellor_counsellee_inline_add.htm";
else
	$templatefile = "counsellor_counsellee_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: counsellor_counsellee_add.php');
...
//	processing CounselleeDevoteeID - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_CounselleeDevoteeID = $pageObject->getControl("CounselleeDevoteeID", $id);
		$control_CounselleeDevoteeID->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing CounselleeDevoteeID - end
...
//	insert masterkey value if exists and if not specified
	if(@$_SESSION[$sessionPrefix."_mastertable"]=="devotee")
	{
		...
		if($avalues["CounsellorDevoteeID"]==""){
			$avalues["CounsellorDevoteeID"] = prepare_for_db("CounsellorDevoteeID",$_SESSION[$sessionPrefix."_masterkey1"]);
		}
...
//	processing CounselleeDevoteeID - start
			...
			if($inlineAddOption)
			{
				$control_CounselleeDevoteeID->afterSuccessfulSave();
			}
//	processing CounselleeDevoteeID - end
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'counsellor_counsellee_edit.php?'.$keylink.'\'>'."Edit".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'counsellor_counsellee_view.php?'.$keylink.'\'>'."View".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: counsellor_counsellee_".$pageObject->getPageType().".php");
...
if(array_key_exists("copyid1",$_REQUEST) || array_key_exists("editid1",$_REQUEST))
{
	...
	{
		$copykeys["CounsellorCounselleeID"]=postvalue("copyid1");
	}
	else
	{
		$copykeys["CounsellorCounselleeID"]=postvalue("editid1");
	}
	...
	if(!$defvalues)
		$defvalues=array();
//	clear key fields
	$defvalues["CounsellorCounselleeID"]="";
...
if(@$_SESSION[$sessionPrefix."_mastertable"]=="devotee")
{
	...
	if(postvalue("mainMPageType")<>"add")
		$defvalues["CounsellorDevoteeID"] = @$_SESSION[$sessionPrefix."_masterkey1"];	
...
if($readavalues)
{
	...
	$defvalues["CounselleeDevoteeID"]=@$avalues["CounselleeDevoteeID"];
}
...
if($inlineadd!=ADD_INLINE)
{
	...
	if(!$pageObject->isAppearOnTabs("CounselleeDevoteeID"))
		$xt->assign("CounselleeDevoteeID_fieldblock",true);
	else
		$xt->assign("CounselleeDevoteeID_tabfieldblock",true);
	$xt->assign("CounselleeDevoteeID_label",true);
	if(isEnableSection508())
		$xt->assign_section("CounselleeDevoteeID_label","<label for=\"".GetInputElementId("CounselleeDevoteeID", $id, PAGE_ADD)."\">","</label>");
...
if(@$_POST["a"]=="added" && ($inlineadd == ADD_INLINE || $inlineadd == ADD_MASTER || $inlineadd==ADD_POPUP)) 
{
	...
	$keylink="";
	$keylink.="&key1=".htmlspecialchars(rawurlencode(@$data["CounsellorCounselleeID"]));
...
//	CounsellorCounselleeID
	...
	if($display)
	{	
		$value = $pageObject->showDBValue("CounsellorCounselleeID", $data, $keylink);
		$showValues["CounsellorCounselleeID"] = $value;
		$showFields[] = "CounsellorCounselleeID";
	}	
//	CounsellorDevoteeID
	$display = false;
...
//	CounselleeDevoteeID
	...
	if($display)
	{	
		$value = $pageObject->showDBValue("CounselleeDevoteeID", $data, $keylink);
		$showValues["CounselleeDevoteeID"] = $value;
		$showFields[] = "CounselleeDevoteeID";
	}	
		$showRawValues["CounsellorCounselleeID"] = substr($data["CounsellorCounselleeID"],0,100);
		$showRawValues["CounsellorDevoteeID"] = substr($data["CounsellorDevoteeID"],0,100);
		$showRawValues["CounselleeDevoteeID"] = substr($data["CounselleeDevoteeID"],0,100);
...
if($pageObject->isShowDetailTables && ($inlineadd==ADD_SIMPLE || $inlineadd==ADD_POPUP) && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "counsellor_counsellee";
		$options['firstTime'] = 1;
